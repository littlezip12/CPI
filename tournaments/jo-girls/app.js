...
function parsePoolPlacement(raw){const value=String(raw||'').trim().replace(/_/g,' '),m=value.match(/^(1st|2nd|3rd)\s+([A-Za-z]{2})\s+([A-Z])(?:\s*[-–—:]\s*.*)?$/i);return m?{rank:Number(m[1][0]),track:m[2].toLowerCase(),pool:m[3].toUpperCase(),key:`${m[2].toLowerCase()}_${m[3].toUpperCase()}`} : null}
function parseGroupPlacement(raw){const value=String(raw||'').trim().replace(/_/g,' '),m=value.match(/^(1st|2nd|3rd)\s+([A-Z])(?:\s*[-–—:]\s*.*)?$/i);return m?{rank:Number(m[1][0]),group:m[2].toUpperCase(),key:m[2].toUpperCase()}:null}
