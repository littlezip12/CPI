(function(){
  ...
  function findHeaderIndex(headers, exactPatterns, suffixWords=[]){
    const exact=findIndex(headers, exactPatterns);
    if(exact>=0) return exact;
    for(const word of suffixWords){
      const pat=new RegExp(`(?:^|\\s)${word.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}\\s*$`,'i');
      const idx=headers.findIndex(h=>pat.test(h));
      if(idx>=0) return idx;
    }
    return -1;
  ...
  function validResultDate(v){ return /^\d{1,2}-[A-Za-z]{3}$/.test(norm(v)) || /^\d{1,2}\/\d{1,2}(?:\/\d{2,4})?$/.test(norm(v)); }
  function validResultTime(v){ return /^\d{1,2}:\d{2}\s*(?:AM|PM)?$/i.test(norm(v)); }
  ...
  function parseRows(rows, tab){
    ...
    rows.forEach(row=>{
      if(isHeaderRow(row)){
        ...
        map={
          date:findHeaderIndex(header,[/^date$/],["date"]),
          time:findHeaderIndex(header,[/^time$/],["time"]),
          venue:findHeaderIndex(header,[/^location$/, /^venue$/, /^site$/, /^pool$/],["location","venue","site"]),
          gameNo:findHeaderIndex(header,[/^gm$/, /^gm #$/, /^game$/, /^game #$/, /^game#$/],["game#","game","gm#","gm"]),
          gmid:findIndex(header,[/gmid|game id|gameid|^id$/]),
          white:findHeaderIndex(header,[/^white$/, /white team/, /^home$/, /^team 1$/, /^team a$/, /^visitor$/],["white"]),
          dark:findHeaderIndex(header,[/^dark$/, /dark team/, /^away$/, /^team 2$/, /^team b$/, /^opponent$/],["dark"]),
          whiteScore:findIndex(header,[/white score|home score|team 1 score|score white|^w score$|^score 1$/]),
          darkScore:findIndex(header,[/dark score|away score|team 2 score|score dark|^d score$|^score 2$/]),
          stage:findHeaderIndex(header,[/^type$/, /^stage$/, /^round$/, /^comments?$/],["comments","comment","stage","round","type"])
        };
        if(map.date<0 && map.venue>=2) map.date=map.venue-2;
        if(map.time<0 && map.venue>=1) map.time=map.venue-1;
        if(map.whiteScore<0 && map.white>=0 && map.dark>map.white+1) map.whiteScore=map.white+1;
        if(map.darkScore<0 && map.dark>=0 && map.dark+1<header.length) map.darkScore=map.dark+1;
        return;
      }
      if(!map) return;
      ...
      const date=get(map.date), time=get(map.time), venue=get(map.venue), gameNo=get(map.gameNo), gmid=get(map.gmid)||gameNo, stage=get(map.stage);
      if(!white && !dark) return;
      if(!isLikelyGameRow(row, map)) return;
      if(!validResultDate(date) || !validResultTime(time)) return;
      const [ws, ds] = scorePairFromRow(row, map);
      out.push({event:state.event.title,group:tab.label,age:tab.age,gender:tab.gender,division:tab.division,date,time,venue,gameNo,gmid,stage,white,dark,whiteScore:ws,darkScore:ds,status:(ws!==null&&ds!==null)?"Final":"Scheduled",source:"live sheet"});
    });
    ...
    if(!out.length){
      rows.forEach(row=>{
        ...
        if(uniqueTeams.length>=2){
          const nums=row.map(asScore).filter(v=>v!==null);
          out.push({event:state.event.title,group:tab.label,age:tab.age,gender:tab.gender,division:tab.division,date:non.find(maybeDate)||"",time:non.find(maybeTime)||"",venue:"",gameNo:non.find(maybeGameId)||"",gmid:non.find(maybeGameId)||"",stage:"",white:uniqueTeams[0],dark:uniqueTeams[1],whiteScore:nums.length>=2?nums[nums.length-2]:null,darkScore:nums.length>=2?nums[nums.length-1]:null,status:nums.length>=2?"Final":"Scheduled",source:"live sheet"});
        }
      });
    }
  ...
  function scoreParts(value){
    ...
    return m?{regulation:Number(m[1]),shootout:Number(m[2])}:{regulation:Number(value),shootout:null};
  }
  function shootoutInfo(g){
    if(g.whiteScore===null||g.darkScore===null||g.whiteScore===undefined||g.darkScore===undefined)return null;
    const w=scoreParts(g.whiteScore),d=scoreParts(g.darkScore);
    return w.shootout!==null&&d.shootout!==null&&w.regulation===d.regulation?{white:w.shootout,dark:d.shootout,regulation:w.regulation}:null;
  }
  function score(g){
    if(g.whiteScore!==undefined&&g.whiteScore!==null&&g.darkScore!==undefined&&g.darkScore!==null){
      const so=shootoutInfo(g);
      return so?`${g.whiteScore}–${g.darkScore} (SO ${so.white}–${so.dark})`:`${g.whiteScore}–${g.darkScore}`;
    }
    if(g.score) return g.score;
    return "—";
  }
  function placementForGame(g){
    const m=norm(g.stage).match(/^(\d+)(?:st|nd|rd|th)$/i);
    if(!m||g.whiteScore===null||g.darkScore===null||g.whiteScore===undefined||g.darkScore===undefined)return null;
    const first=Number(m[1]),whiteWon=g.whiteScore>g.darkScore;
    return {winner:first,loser:first+1,white:whiteWon?first:first+1,dark:whiteWon?first+1:first};
  }
  function finishForTeam(g,team){
    const p=placementForGame(g);if(!p)return null;
    return sortTeamName(g.white)===sortTeamName(team)?p.white:sortTeamName(g.dark)===sortTeamName(team)?p.dark:null;
  }
  ...
  function teamGames(team){
    const key=sortTeamName(team),group=$("groupFilter").value;
    return state.games.filter(g=>(!group||belongsToGroup(g,group))&&(sortTeamName(g.white)===key || sortTeamName(g.dark)===key));
  }
  ...
  function renderTeamJourney(){
    ...
    const finish=games.map(g=>finishForTeam(g,team)).find(Boolean);
    ...
    const rows=games.map(g=>{
      ...
      const placement=finishForTeam(g,team);
      return `<article class="tr-journey-game ${cls}"><div><strong>${esc(result)}${placement?` · ${esc(ordinal(placement))}`:""}</strong><span>${esc(g.gameNo||g.gmid||"Game")} · ${esc(g.date||"")} ${esc(g.time||"")} · ${esc(g.venue||"")}${g.stage?` · ${esc(g.stage)}`:""}</span></div><div><b>${esc(team)}</b> vs ${esc(opp||"TBD")}</div><div class="tr-journey-score">${esc(score(g))}</div></article>`;
    }).join("");
    panel.innerHTML=`<div class="tr-journey-head"><div><p class="tr-eyebrow">Team results</p><h2>${esc(team)}</h2><p>${esc(groups || "All loaded divisions")}${finish?` · Finished ${esc(ordinal(finish))}`:""}</p></div><div class="tr-record-badge"><span>Record</span><strong>${wins}-${losses}</strong></div></div>${scheduled?`<div class="tr-next-card"><span>Next scheduled game</span><strong>${esc(scheduled.gameNo||scheduled.gmid||"Game")} · ${esc(scheduled.date||"")} ${esc(scheduled.time||"")}</strong><em>${esc(scheduled.venue||"")}</em></div>`:""}<div class="tr-journey-list">${rows || `<div class="tr-empty">No game rows found for this team in the loaded tournament data.</div>`}</div>`;
  }
