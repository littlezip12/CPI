...
function resolveTournament(){
  ...
  return{games,map,slots,placements:new Map([...groupPlacements,...poolPlacements])}
}
...
function renderFullSchedule(){
  ...
  fullSchedule.innerHTML=list.length?list.map(g=>`<article class="game glass ${isFinal(g)?'completed':''}"><div class="top"><span>Game ${g.game}${isFinal(g)?' · FINAL':''}</span><span>${esc(friendlyDate(g.date))} · ${esc(g.time)}</span></div><div class="match">${esc(display(g,'white'))} ${isFinal(g)?`<span class="score">${esc(g.whiteScore)}–${esc(g.darkScore)}</span>`:'<span style="color:var(--muted)">vs</span>'} ${esc(display(g,'dark'))}</div><div class="journey-meta">${esc(g.location)} · ${esc(g.type)} · ${esc(g.gmid)}</div></article>`).join(''):'<div class="empty glass">No games match the current full-schedule filters.</div>';
}
...
function renderTeam(){const name=team.value,config=currentConfig();if(!name){$('teamView').classList.add('hidden');setEmptyState();updateShareUrl();return}$('teamView').classList.remove('hidden');setEmptyState();updateShareUrl();if(config)localStorage.setItem(`joBoysSelectedTeam:${config.id}`,name);const games=gamesForTeam(name),completed=games.filter(isFinal),upcoming=games.find(g=>!isFinal(g));const wins=completed.filter(g=>resultFor(g,name)==='win').length,losses=completed.length-wins;summary.innerHTML=`<div class="eyebrow">Selected team</div><h2>${esc(name)}</h2><div class="stats"><div class="stat"><small>Record</small>${wins}-${losses}</div><div class="stat"><small>Games played</small>${completed.length}</div><div class="stat"><small>Status</small>${esc(statusText(games,upcoming))}</div></div>`;if(upcoming){const opp=otherTeam(upcoming,name),candidates=gameCandidates(upcoming,name);next.innerHTML=`<div class="next-label">Next game</div><div class="next-match">${esc(name)} vs ${esc(opp||candidates.join(' or ')||'TBD')}</div><div>${esc(friendlyDate(upcoming.date))} · ${esc(upcoming.time)}</div><div class="journey-meta">${esc(upcoming.location)} · Game ${upcoming.game} · ${esc(upcoming.type)}</div>`;renderPaths(upcoming)}else{next.innerHTML='<div class="next-label">Tournament status</div><div class="next-match">No upcoming game</div>';renderPaths({winnerTo:'',loserTo:''})}const grouped={};games.forEach(g=>(grouped[g.date]??=[]).push(g));journey.innerHTML=games.length?'<div class="journey">'+Object.entries(grouped).map(([date,list])=>`<section class="day"><h3>${esc(friendlyDate(date))}</h3>${list.map(g=>{const r=resultFor(g,name)||'upcoming',opp=otherTeam(g,name)||gameCandidates(g,name).join(' or ')||'TBD';const text=isFinal(g)?(g.whiteTeam===name?`${name} ${g.whiteScore}–${g.darkScore} ${opp}`:`${opp} ${g.whiteScore}–${g.darkScore} ${name}`):`${name} vs ${opp}`;return`<article class="journey-card glass ${r}"><div class="journey-time">Game ${g.game}<br>${esc(g.time)}</div><div><div class="journey-score">${esc(text)}</div><div class="journey-meta">${esc(g.location)} · ${esc(g.type)}</div></div><span class="pill ${r}">${r==='win'?'Win':r==='loss'?'Loss':'Upcoming'}</span></article>`}).join('')}</section>`).join('')+'</div>':'<div class="empty glass">No resolved games are currently available for this team.</div>';renderRelevant();renderPotential(name,upcoming)}
...
function renderRelevant(){const name=team.value,q=String(search.value||'').trim().toLowerCase(),selectedDay=day.value,games=gamesForTeam(name),upcoming=games.find(g=>!isFinal(g)),ids=relevantIds(name,upcoming);let list=RESOLVED.games.filter(g=>g.whiteTeam||g.darkTeam);if(!q)list=list.filter(g=>ids.has(g.game));list=list.filter(g=>(!selectedDay||g.date===selectedDay)&&(!q||[display(g,'white'),display(g,'dark'),g.location,g.type,g.gmid,g.game].join(' ').toLowerCase().includes(q))).sort(gameSort);schedule.innerHTML=list.length?list.map(g=>`<article class="game glass ${isFinal(g)?'completed':''}"><div class="top"><span>Game ${g.game}${isFinal(g)?' · FINAL':''}</span><span>${esc(friendlyDate(g.date))} · ${esc(g.time)}</span></div><div class="match">${esc(display(g,'white'))} ${isFinal(g)?`<span class="score">${esc(g.whiteScore)}–${esc(g.darkScore)}</span>`:'<span style="color:var(--muted)">vs</span>'} ${esc(display(g,'dark'))}</div><div class="journey-meta">${esc(g.location)} · ${esc(g.type)}</div></article>`).join(''):'<div class="empty glass">No games match the current filters.</div>'}
function renderPotential(name,upcoming){const candidates=new Set(gameCandidates(upcoming,name));if(upcoming){for(const dest of [upcoming.winnerTo,upcoming.loserTo]){const g=targetGame(dest);gameCandidates(g,name).forEach(x=>candidates.add(x))}}candidates.delete(name);potential.innerHTML=candidates.size?[...candidates].filter(x=>DATA.teams.includes(x)).map(x=>`<button data-team="${esc(x)}"><strong>${esc(x)}</strong><div class="journey-meta">View tournament journey</div></button>`).join(''):'<div class="empty glass">No additional future opponents are currently identifiable.</div>';potential.querySelectorAll('[data-team]').forEach(button=>button.addEventListener('click',()=>{team.value=button.dataset.team;renderTeam();window.scrollTo({top:0,behavior:'smooth'})}))}
