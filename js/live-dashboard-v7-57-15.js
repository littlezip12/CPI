 ...
 * Product-surface release only: the connected backend and role contracts remain unchanged.
 */
(() => {
  "use strict";
  ...
  function renderGameSeriesArchive() {
    ...
    if (!container) return;
    if (!gameSeriesArchive.length) {
      container.innerHTML = '<p class="live-empty-state">No tournaments or scrimmage weekends yet.</p>';
      return;
    }
    container.innerHTML = gameSeriesArchive.map(series => {
      const label = series.seriesType === "tournament" ? "Tournament" : "Scrimmage weekend";
      const record = `${Number(series.wins || 0)}-${Number(series.losses || 0)}${Number(series.ties || 0) ? `-${Number(series.ties)}` : ""}`;
      const gamesHtml = (series.games || []).map(game => `<div class="live-history-row"><div><strong>${escapeHtml(game.teamName || workspace?.teamName || "Team")} vs ${escapeHtml(game.opponentName || "Opponent")}</strong><span>${escapeHtml(gameDayTimeLabel(game.scheduledAt))}${game.venue ? ` · ${escapeHtml(game.venue)}` : ""}</span></div><div class="live-history-score"><strong>${escapeHtml(String(game.teamScore ?? 0))}–${escapeHtml(String(game.opponentScore ?? 0))}</strong><span>Final</span></div><div class="live-dashboard-actions"><a href="${escapeHtml(teamScopedUrl("live-game.html", {game:game.id}))}">View</a></div></div>`).join("");
      return `<article class="live-game-series-card"><div class="live-panel-heading"><div><p class="live-section-kicker">${escapeHtml(label)}</p><h3>${escapeHtml(series.name)}</h3></div><div class="live-game-series-record"><strong>${escapeHtml(record)}</strong><span>${Number(series.finalCount || 0)} final${Number(series.finalCount || 0) === 1 ? "" : "s"}</span></div></div>${gamesHtml || '<p class="live-empty-state">No completed games in this weekend yet.</p>'}</article>`;
    }).join("");
  }

  async function loadGameSeriesArchive() {
    if (!backend || !workspace?.teamId) { gameSeriesArchive = []; renderGameSeriesArchive(); return; }
    const {data,error} = await backend.client.rpc("live_game_series_archive_v1", {target_team_id:workspace.teamId});
