/* WPI 7.64.27 — My Teams follow persistence + Water Polo HQ supporter copy. */
(() => {
  "use strict";
  const RELEASE="7.64.27";
  ...
  const requestedFollowTeam=()=>String(params.get("followTeam")||"").trim();

  function loginHrefForFollow(){
    const url=new URL("live-login.html",window.location.href);
    url.searchParams.set("follow","1");
    const teamId=requestedFollowTeam();if(teamId)url.searchParams.set("followTeam",teamId);
  ...
  async function loadDirectory(){
    ...
    if(!res.ok)throw new Error("Water Polo HQ team directory could not load");
  ...
  function renderMyTeams(){
    ...
    $("myTeams").innerHTML=rows.length?rows.map(r=>{
      ...
      return `<article class="my-team my-team--dashboard${s.live?" is-live":""}">
        <header><img src="${esc(logoFor(r))}" alt=""><div><strong>${esc(r.label)}</strong><small>${esc(r.meta||"Water Polo HQ team")}</small></div><span class="follow-pill" data-state="${esc(r.relationship)}">${r.relationship==="member"?"Member":"Following"}</span></header>
  ...
  function renderDirectory(){
    ...
    const hasFilter=Boolean(q||club!=="all"||age!=="all"||gender!=="all"||orgType!=="all"),rows=hasFilter?filtered.slice(0,60):[];
    if(!hasFilter){$("directoryResults").innerHTML='<div class="follow-discovery-empty"><strong>Start with your team name.</strong><span>Type a club, school or team above. Water Polo HQ will narrow the directory as you type instead of showing every team at once.</span></div>';$("directoryResultNote").textContent=`${filtered.length} teams available · search to narrow`;return;}
    $("directoryResults").innerHTML=rows.length?rows.map(t=>{const isFollowed=followed.has(String(t.familyKey)),live=liveMap.get(String(t.familyKey)),isMember=members.has(String(t.familyKey));return `<article class="directory-team"><header><img src="${esc(logoFor(t))}" alt=""><div><strong>${esc(t.clubName)} · ${esc(t.teamName)}</strong><small>${esc([t.ageGroup,t.gender,t.squadDescriptor].filter(Boolean).join(" · "))}</small></div></header><footer><span class="follow-pill" data-state="${live?"member":"waiting"}">${live?"Water Polo HQ Live":"Directory team"}</span><span class="follow-card-actions"><a class="follow-team-open" href="${esc(t.teamHubHref)}">Team page</a>${isMember?`<button type="button" disabled>Member</button>`:`<button type="button" data-family-follow="${esc(t.familyKey)}" data-following="${isFollowed}" aria-pressed="${isFollowed}">${isFollowed?"Following":"Follow"}</button>`}</span></footer></article>`;}).join(""):'<p class="follow-empty">No Water Polo HQ teams match those filters.</p>';
    $("directoryResultNote").textContent=`${filtered.length} match${filtered.length===1?"":"es"}`;
  }
  ...
  async function init(){document.body.dataset.liveSupporterHubRelease=RELEASE;try{await loadDirectory();if(!window.WPILiveBackend?.isConfigured(config))throw new Error("Water Polo HQ Live is not configured on this deployment.");backend=await window.WPILiveBackend.connect(config);session=await backend.session();if(!session||backend.isAnonymousUser(session.user)){if(session)await backend.signOut();$("followSignedOut").hidden=false;const link=$("followSignInLink");if(link)link.href=loginHrefForFollow();if(requestedFollowTeam()){const h=$("followSignedOut").querySelector("h2"),p=$("followSignedOut").querySelector("p");if(h)h.textContent="Sign in to follow this team";if(p)p.textContent="This team link will be waiting after sign-in. Following is read-only and never grants scoring or admin access.";}return;}document.querySelectorAll("[data-follow-auth-only]").forEach(el=>{el.hidden=false;});$("followConnected").hidden=false;await loadOverview();await applySharedTeamLink();}catch(e){$("followSignedOut").hidden=false;$("followSignedOut").querySelector("h2").textContent="Supporter Hub could not connect";$("followSignedOut").querySelector("p").textContent=e.message||"Connection failed.";}}
