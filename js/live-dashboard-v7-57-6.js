...
(() => {
  ...
  function catalogTeamMatch(name) {
    ...
    const candidates = gameCatalogTeams.filter(team => {
      ...
      return normalizedGameTeamName(team.team) === normalized || normalizedGameTeamName(`${team.displayClubName || team.club || ""} ${team.team || ""}`) === normalized;
    });
    return candidates[0] || gameCatalogTeams.find(team => normalizedGameTeamName(team.team) === normalized) || null;
  }

  function currentTeamCatalogMatch() {
    return catalogTeamMatch(workspace?.teamName || "");
  }
  ...
  async function loadGameCatalog() {
    ...
    try {
      const [clubsResponse,tournamentsResponse] = await Promise.all([
        fetch("clubs.json", {cache:"no-store"}),
        fetch("data/tournaments/public-hub.json", {cache:"no-store"})
      ]);
      if (clubsResponse.ok) {
        const clubs = await clubsResponse.json();
        gameCatalogTeams = (Array.isArray(clubs) ? clubs : []).flatMap(club => (club.teams || []).map(team => ({
          ...team,
  ...
  function updateGameMatchupPreview() {
    const opponentName = $("gameOpponentName")?.value.trim() || "Opponent";
    const opponent = catalogTeamMatch(opponentName);
    const own = currentTeamCatalogMatch();
    $("gameTeamPreviewName").textContent = workspace?.teamName || "Your team";
    $("gameOpponentPreviewName").textContent = opponentName;
    $("gameTeamLogoPreview").src = safeGameLogo(own?.logo);
    $("gameOpponentLogoPreview").src = safeGameLogo(opponent?.logo);
  ...
  async function openGameDayDialog(game = null) {
    ...
    $("gameDayDialogHelp").textContent = game
      ...
      : "Choose the kind of game. Tournament games remain eligible for official schedule reconciliation even when you have to add them manually.";
    const kind = game?.gameKind || "tournament";
    const kindInput = document.querySelector(`input[name="gameKind"][value="${CSS.escape(kind)}"]`);
    if (kindInput) kindInput.checked = true;
    $("gameOpponentName").value = game?.opponentName || "";
  ...
  function gameDayFormPayload() {
    ...
    return {
      ...
      teamLogoUrl:own?.logo || null,
      opponentLogoUrl:opponent?.logo || null
    };
  }
  ...
  async function saveGameDay({startAfter=false} = {}) {
    ...
    try {
      ...
      let result;
      if (editingGameDayId) {
        const {data,error} = await backend.client.rpc("live_update_planned_game_v1", {target_game_id:editingGameDayId,...rpcArgs});
        if (error) throw error;
        result = data;
      } else {
        const {data,error} = await backend.client.rpc("live_create_manual_game_v2", {target_team_id:workspace.teamId,...rpcArgs});
        if (error) throw error;
        result = data;
      }
  ...
  function gameDaySourceLabel(game) {
    if (game.gameKind === "tournament") {
      if (game.reconciliationStatus === "verified") return "Official tournament link verified";
      if (game.creationSource === "tournament_schedule") return "WPI tournament schedule";
      return "Manual tournament · official link pending";
    }
    return game.gameKind === "friendly" ? "Manual friendly" : "Manual scrimmage";
  }
  ...
  function renderGameDayHub() {
    ...
    $("gameDayQueueSummary").textContent = deck.length
      ? "Upcoming, ready and live games stay here. Final games move to History."
      : "No upcoming games yet. Add a tournament game, scrimmage or friendly.";
    if (!deck.length) {
      container.innerHTML = `<div class="live-game-day-empty"><strong>No games on deck</strong><span>Add a tournament game even if the official WPI schedule is missing, or create a scrimmage/friendly manually.</span>${["owner","admin"].includes(workspace.role) ? '<button type="button" data-game-day-add>Add first game</button>' : ""}</div>`;
      return;
    }
    container.innerHTML = deck.map(game => {
      ...
      const canOpen = game.status === "live" || canManage;
      const tournament = game.gameKind === "tournament" && game.tournamentName ? `<strong class="live-game-day-tournament">${escapeHtml(game.tournamentName)}</strong>` : `<strong class="live-game-day-tournament">${escapeHtml(game.gameKind === "friendly" ? "Friendly" : "Scrimmage")}</strong>`;
      return `<article class="live-game-day-card" data-game-day-id="${escapeHtml(game.id)}">
        ...
        <div class="live-game-day-matchup">
          <div class="live-game-day-team"><img src="${escapeHtml(safeGameLogo(game.teamLogoUrl || currentTeamCatalogMatch()?.logo))}" alt="${escapeHtml(game.teamName)} logo" onerror="this.onerror=null;this.src='assets/branding/wpi-logo-mark.png'"><span>${escapeHtml(game.teamName)}</span></div>
          <strong>vs</strong>
          <div class="live-game-day-team live-game-day-team--opponent"><img src="${escapeHtml(safeGameLogo(game.opponentLogoUrl))}" alt="${escapeHtml(game.opponentName)} logo" onerror="this.onerror=null;this.src='assets/branding/wpi-logo-mark.png'"><span>${escapeHtml(game.opponentName)}</span></div>
        </div>
  ...
  async function loadGameDayQueue() {
    ...
    const {data,error} = await backend.client.rpc("live_game_day_queue", {target_team_id:workspace.teamId});
    if (error) throw error;
  ...
  $("refreshGameDayButton").addEventListener("click", () => loadGames().catch(error => { $("dashboardConnectionDetail").textContent = error.message; }));
  ...
  $("gameDayQueue").addEventListener("click", event => {
    ...
    if (start) return void window.location.assign(teamScopedUrl("live-sandbox.html", {game:start.dataset.gameDayStart}));
