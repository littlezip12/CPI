/* WPI 7.56.6 — scoped team operations, GroupMe administration, and game-scoped scorer control. */
(() => {
  "use strict";
  ...
  async function init() {
    ...
    if (!window.WPILiveBackend?.isConfigured(config)) {
      ...
      try { demoSession = JSON.parse(localStorage.getItem("wpi-live-auth-v7-56-6") || localStorage.getItem("wpi-live-auth-v7-56-3") || localStorage.getItem("wpi-live-auth-v7-56-1") || "null"); } catch (_) { demoSession = null; }
  ...
  $("dashboardSignOutButton").addEventListener("click", async () => {
    ...
    localStorage.removeItem("wpi-live-auth-v7-56-6");
