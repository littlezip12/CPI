/* WPI 7.56.3 — connected live backend with reliable event-delivery dispatch.
  ...
  class WPILiveBackend {
    ...
    static async connect(config = {}) {
      ...
      const client = module.createClient(config.supabaseUrl, config.supabasePublishableKey, {
        ...
        global: { headers: { "x-wpi-live-release": config.release || "7.56.3" } }
      });
    ...
    async syncState(state) {
      ...
      let remoteEventMap = Object.fromEntries(upsertedEvents.map(event => [event.client_event_id,event.id]));
