/* WPI 7.61.0 — Supporter Hub. Unified club + high-school organization discovery. */
(() => {
  "use strict";
  ...
  async function loadDirectory(){const [clubRes,schoolRes]=await Promise.all([fetch("data/live/team-identity-directory-v7-60-2.json?v=7.61.0",{cache:"no-store"}),fetch("data/live/high-school-directory-v7-61-0.json?v=7.61.0",{cache:"no-store"})]);if(!clubRes.ok||!schoolRes.ok)throw new Error("WPI organization directory could not load");const clubDir=await clubRes.json(),schoolDir=await schoolRes.json();const clubOrgs=(clubDir.clubs||[]).map(c=>({...c,organizationType:"club"}));const schoolOrgs=(schoolDir.organizations||[]).map(o=>({clubId:o.organizationId,name:o.name,logo:o.logo,organizationType:"high_school"}));const clubTeams=(clubDir.teams||[]).map(t=>({...t,organizationType:"club"}));const schoolTeams=(schoolDir.teams||[]).map(t=>({...t,organizationType:"high_school"}));directory={clubs:[...clubOrgs,...schoolOrgs],teams:[...clubTeams,...schoolTeams]};renderDiscoveryFilters();}
