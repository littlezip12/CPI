/* WPI 7.56.13 — Game Actions & Automatic GroupMe Summary built on the validated 7.56.12 game-day baseline.
...
(() => {
  ...
  function renderEventVariants(action = null) {
    ...
    chooser.innerHTML = `<span>Which one?</span><div>${action.variants.map(eventId => {
      const active = $("eventType")?.value === eventId;
      return `<button type="button" data-event-variant="${eventId}" class="${active ? "is-active" : ""}">${escapeHtml(QUICK_EVENT_VARIANT_LABELS[eventId] || EVENT_MAP.get(eventId)?.label || eventId)}</button>`;
    }).join("")}</div>`;
  ...
  function renderTimeline() {
    ...
    $("timelineList").innerHTML = events.map(event => {
      ...
      if (event.type === "game_summary") detail = "Automatically queued after Final Whistle";
  ...
  function buildGroupMeSummary(stats) {
    ...
    const leaders = [...stats.players.values()].map(item => {
      ...
      const saves = item.counts.save || 0;
      const steals = item.counts.steal || 0;
      const blocks = item.counts.field_block || 0;
      const assistsCount = item.assists || 0;
      const impact = goals + assistsCount + saves + steals + blocks;
      const details = [];
      if (goals) details.push(`${goals}G`);
      if (assistsCount) details.push(`${assistsCount}A`);
      if (saves) details.push(`${saves} save${saves === 1 ? "" : "s"}`);
      if (steals) details.push(`${steals} steal${steals === 1 ? "" : "s"}`);
      if (blocks) details.push(`${blocks} block${blocks === 1 ? "" : "s"}`);
      return {label:playerLabel(item.player), impact, details};
    }).filter(item => item.impact > 0).sort((a,b) => b.impact - a.impact).slice(0,3);
    const lines = [
      "GAME SUMMARY",
      ...
      `Turnovers ${stats.counts.turnover || 0} · Exclusions drawn ${stats.counts.exclusion_drawn || 0} · 5M drawn ${stats.counts.five_meter_drawn || 0}`
    ];
    if (leaders.length) {
      lines.push("", "Top contributors");
      for (const item of leaders) lines.push(`${item.label} — ${item.details.join(" · ")}`);
    }
  ...
  function ensureAutomaticGameSummary() {
    const existing = activeEvents().find(event => event.type === "game_summary");
    ...
    const event = baseEvent(type, {
      ...
      timeRemaining:"0:00",
      note:buildGroupMeSummary(stats)
    });
    state.game.events.push(event);
    event.scoreAfter = {team:state.game.teamScore, opponent:state.game.opponentScore};
    createMessage(event, type);
  ...
  async function endGame({skipConfirm = false} = {}) {
    ...
    try {
      ...
      if (backend && workspace) {
        ...
        const summaryEvent = activeEvents().find(event => event.type === "game_summary");
        const summaryMessage = summaryEvent ? state.game.messages.find(message => message.eventId === summaryEvent.id) : null;
        setSyncStatus(summaryMessage?.status === "sent" ? "Final saved · Summary sent" : "Final saved", "saved");
      }
