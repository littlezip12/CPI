/* WPI 7.56.5 — connected live backend with reliable event-delivery dispatch.
  ...
  class WPILiveBackend {
    ...
    static async connect(config = {}) {
      ...
      const client = module.createClient(config.supabaseUrl, config.supabasePublishableKey, {
        ...
        global: { headers: { "x-wpi-live-release": config.release || "7.56.5" } }
      });
    ...
    async syncState(state) {
      ...
      // Cross-account scorers must update an existing game rather than issue an
      // INSERT ... ON CONFLICT request. PostgreSQL evaluates the INSERT policy
      // before the conflict update, and the original game's created_by belongs
      // to the Owner. A Scorer therefore received an RLS conflict before any
      // event or GroupMe delivery could be persisted.
