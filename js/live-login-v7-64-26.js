/* WPI 7.64.26 — Native-capable passwordless Supporter Auth Entry.
...
(() => {
  ...
  function setMode(nextMode) {
    if (nextMode === "signup" && !signupAllowed) {
      $("loginMessage").textContent = onboarding ? "Create an account to submit a club onboarding request." : (following ? "Create a read-only supporter account to follow WPI teams." : "New team accounts require a private team invitation.");
      return;
    }
    mode = nextMode;
    ...
    if (passwordlessSupporter) {
      ...
      $("loginExplanation").textContent = "Enter your email. WPI will send a one-time secure sign-in link—no password required.";
    ...
    $("loginExplanation").textContent = signingUp
      ...
      : (onboarding ? "Sign in to submit or review a club onboarding request." : (following ? "Sign in to open My Teams and your read-only WPI game feed." : "Sign in once, then choose any team workspace your account can access."));
    $("loginMessage").textContent = "";
  }
  ...
  function init() {
    ...
    window.addEventListener("wphq:native-auth-url", () => {
      if (backend) completeNativeAuthIfPresent().catch(error => { if ($("loginMessage")) $("loginMessage").textContent = error.message; });
    });
    setMode("signin");
    initConnectedMode();
  }
