 ...
 * Adds Owner/Admin evidence for the remaining real-world Club Pilot gates without
 * changing validated scorer, GroupMe, Following, archive or tournament semantics.
 *
...
(() => {
  ...
  function pilotStateLabel(state) {
    return ({observed:"Evidence found",guarded:"Guarded",manual:"Run test",external:"Waiting",attention:"Attention",pending:"Not yet observed"})[state] || "Pending";
  }
  ...
  function renderClubPilotValidation() {
    ...
    const next = gates.find(gate => ["attention","pending","manual"].includes(gate.state)) || gates.find(gate => gate.state === "external");
    ...
    if (nextBox) {
      if (next) {
        ...
        nextBox.innerHTML = `<strong>Engineered evidence is complete</strong><span>Review the external tournament-feed gate before deciding whether 7.59.0 is justified.</span>`;
      }
    }

    const gateList = $("clubPilotGateList");
    if (gateList) gateList.innerHTML = gates.length ? gates.map(gate => `<article class="live-pilot-gate" data-state="${escapeHtml(gate.state || "pending")}"><span class="live-pilot-gate-mark" aria-hidden="true">${gate.state === "observed" || gate.state === "guarded" ? "✓" : gate.state === "attention" ? "!" : "•"}</span><div><strong>${escapeHtml(gate.title || "Pilot gate")}</strong><p>${escapeHtml(pilotGateDetail(gate))}</p></div><em>${escapeHtml(pilotStateLabel(gate.state))}</em></article>`).join("") : '<p class="live-empty-state">Pilot evidence is not available yet.</p>';
  ...
  async function loadClubPilotValidation() {
    ...
    try {
      ...
      const nextBox = $("clubPilotNextTest");
      if (nextBox) { nextBox.dataset.state = "attention"; nextBox.innerHTML = `<strong>Validation data unavailable</strong><span>Apply the 7.58.7 Supabase migration, then refresh.</span>`; }
    } finally {
