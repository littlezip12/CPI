/* WPI 7.64.15 — Parent privacy and passwordless Supporter Auth Entry.
  ...
  async function submitAuth(event) {
    ...
    if (passwordlessSupporter) {
      ...
      try {
        ...
        const options = { shouldCreateUser:true, emailRedirectTo:redirect.href };
        if (captchaToken) options.captchaToken = captchaToken;
        const {error} = await backend.client.auth.signInWithOtp({email,options});
        if (error) throw error;
        $("loginMessage").textContent = "Check your email for your one-time WPI sign-in link.";
      } catch (error) {
