/* WPI 7.56.2 — private team dashboard with GroupMe connection and delivery audit. */
(() => {
  "use strict";
  ...
  async function init() {
    ...
    if (!window.WPILiveBackend?.isConfigured(config)) {
      ...
      try { demoSession = JSON.parse(localStorage.getItem("wpi-live-auth-v7-56-2") || localStorage.getItem("wpi-live-auth-v7-56-1") || "null"); } catch (_) { demoSession = null; }
  ...
  $("dashboardSignOutButton").addEventListener("click", async () => {
    ...
    localStorage.removeItem("wpi-live-auth-v7-56-2");
