...
(() => {
  ...
  function renderReadinessGuidance(model) {
    ...
    if (allReady) {
      ...
      nextButton.textContent = ["owner","admin"].includes(workspace.role) ? "Start a new game" : "Setup complete";
    ...
    if (launchLink && ["owner","admin"].includes(workspace.role)) {
      ...
      launchLink.textContent = allReady ? "Start a new game" : "Start game before setup is complete";
    }
  ...
  $("readinessNextButton").addEventListener("click", event => {
    ...
    if (button.dataset.mode === "game") {
      window.location.assign(teamScopedUrl("live-sandbox.html", {new:"1"}));
      return;
    }
