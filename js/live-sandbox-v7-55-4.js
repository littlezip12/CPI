/* WPI 7.55.4 — compact dropdown live-scoring sandbox.
...
(() => {
  ...
  const RELEASE = "7.55.4";
  const STORAGE_KEY = "wpi-live-sandbox-v7-55-4";
  const AUTH_KEY = "wpi-live-auth-v7-55-4";
  ...
  const EVENT_TYPES = [
    {id:"goal", label:"Goal", category:"scoring", requiresPlayer:true, allowsAssist:true, teamDelta:1, priority:"major"},
    {id:"opponent_goal", label:"Opponent goal", category:"scoring", requiresPlayer:false, opponentDelta:1, priority:"major"},
  ...
  function renderEventOptions() {
    $("eventType").innerHTML = `<option value="">Select event</option>${EVENT_TYPES.map(type => `<option value="${type.id}">${escapeHtml(type.label)}</option>`).join("")}`;
  }
  ...
  function updateEventFields() {
    ...
    $("primaryPlayerLabel").hidden = Boolean(type && !type.requiresPlayer);
    $("primaryPlayer").disabled = !requiresPlayer;
    $("primaryPlayer").required = requiresPlayer;
    $("assistPlayerLabel").hidden = !(type && type.allowsAssist);
    $("assistPlayer").disabled = !(type && type.allowsAssist);
    if (!(type && type.allowsAssist)) $("assistPlayer").value = "";
  ...
  function updateMessagePreview() {
    ...
    $("messagePreview").textContent = event ? formatMessage(event).replace(/\*\*/g, "") : "Select an event to preview the parent update.";
  }
  ...
  function renderScoreboard() {
    ...
    $("startQuarterButton").textContent = `Set ${quarterText(Number($("currentQuarter").value))} starters`;
  }
  ...
  async function authorize() {
    ...
    if (!connected) {
      ...
      try { demoSession = JSON.parse(localStorage.getItem(AUTH_KEY) || "null"); } catch (_) { demoSession = null; }
  ...
  function bindEvents() {
    ...
    $("primaryPlayer").addEventListener("change", updateMessagePreview);
    $("assistPlayer").addEventListener("change", updateMessagePreview);
    $("eventNote").addEventListener("input", updateMessagePreview);
    $("currentQuarter").addEventListener("change", () => {
      state.game.quarter = Number($("currentQuarter").value);
      $("startQuarterButton").textContent = `Set ${quarterText(state.game.quarter)} starters`;
      saveState();
      renderScoreboard();
      updateMessagePreview();
    });
    $("clockTime").addEventListener("input", updateMessagePreview);
