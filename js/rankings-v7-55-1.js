/* WPI 7.55.1 — season-aware rankings with immutable final snapshot */
(function () {
  const season = window.WPISeason?.resolve("rankings") || { id:"2025-2026", label:"2025–2026", status:"final", rankingStatus:"final" };
  let rankings = [];
  ...
  function teamUrl(team) {
    const href = team.teamPage || `team.html?team=${encodeURIComponent(team.slug || team.team || "")}`;
    return window.WPISeason?.withSeason(href, season.id) || href;
  }

  function clubUrl(team) {
    const href = team.clubPage || `club.html?club=${encodeURIComponent(team.clubSlug || team.club || "")}`;
    return window.WPISeason?.withSeason(href, season.id) || href;
  }
  ...
  function groupOptions() {
    ...
    const groups = [...new Set((rankings.length ? rankings.map(item => item.group) : (window.CPI_PLATFORM?.activeGroups || [])).filter(Boolean))];
  ...
  function render() {
    ...
    if (!els.list) return;
    if (!items.length) {
      if (season.rankingStatus === "results_gathering") {
        const finalHref = `rankings.html?season=2025-2026${group ? `&group=${encodeURIComponent(slugify(group))}` : ""}`;
        els.list.innerHTML = `<div class="rankings-empty wpi-season-empty"><span>2026–2027 season</span><h3>Results gathering in progress</h3><p>No preseason rankings are being fabricated. WPI will publish 2026–2027 rankings only after verified tournament evidence is reviewed.</p><a href="${escapeHtml(finalHref)}">View the 2025–2026 Final Rankings →</a></div>`;
        if (els.count) els.count.textContent = "No 2026–2027 rankings published";
        if (els.heroGroup) els.heroGroup.textContent = `${group || "WPI"} · Results gathering in progress`;
      } else {
  ...
  async function loadSeasonRankings() {
    if (season.rankingStatus !== "final" || !season.rankingsPath) return [];
    try {
      const response = await fetch(`${season.rankingsPath}?v=7.55.1`, { cache:"no-store" });
      if (!response.ok) throw new Error(`Unable to load ${season.rankingsPath}`);
      const data = await response.json();
      if (!Array.isArray(data) || data.length !== 724) throw new Error("Final ranking snapshot is incomplete");
      return data;
    } catch (error) {
      console.error(error);
      return Array.isArray(window.CPI_RANKINGS) ? window.CPI_RANKINGS.filter(item => item.competitiveSeason === season.id || season.id === "2025-2026") : [];
    }
  }

  async function init() {
    rankings = await loadSeasonRankings();
    const seasonFact = document.querySelector("#heroSeason");
    const seasonStatus = document.querySelector("#heroSeasonStatus");
    if (seasonFact) seasonFact.textContent = season.label || season.id;
    if (seasonStatus) seasonStatus.textContent = season.rankingStatus === "final" ? "Final rankings" : "Results gathering";
    document.title = season.rankingStatus === "final" ? `${season.label} Final Rankings | Water Polo Index` : `${season.label} Rankings | Water Polo Index`;
    populateFilters();
