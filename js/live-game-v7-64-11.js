 ...
 * Builds on the protected WPI Live scorer architecture with explicit game participation,
 * lower-friction routine timing, edit/recovery controls, and reliable final delivery.
 *
 * WPI 7.58.6 — Club-Level Pilot Hardening.
 * Preserves the validated scoring, handoff, recovery and GroupMe architecture while
 * isolating each live game's on-device/offline state so concurrent club games cannot
 * overwrite one another in browser storage.
 */
(() => {
  "use strict";

  const RELEASE = "7.56.15";
  const LEGACY_STORAGE_KEY = "wpi-live-sandbox-v7-56-15";
  ...
  const GameStorage = window.WPILiveGameStorage7586;
  const EXACT_TIME_EVENT_TYPES = new Set(["goal","opponent_goal","exclusion_drawn","exclusion_committed","five_meter_drawn","five_meter_committed"]);
  const SYSTEM_EVENT_TYPES = new Set(["quarter_start","quarter_end","overtime_start","shootout_start","game_summary","stat_correction"]);

  function routeRemoteGameId() {
    try { return new URLSearchParams(window.location.search).get("game") || null; }
    catch (_) { return null; }
  }

  function routeTeamId() {
    try { return new URLSearchParams(window.location.search).get("team") || null; }
    catch (_) { return null; }
  }

  function scopedStateStorageKey(candidate = null) {
    const remoteGameId = candidate?.game?.remoteId || routeRemoteGameId();
    if (GameStorage?.gameKey && remoteGameId) return GameStorage.gameKey(remoteGameId);
    if (GameStorage?.draftKey) return GameStorage.draftKey(routeTeamId());
    return LEGACY_STORAGE_KEY;
  }

  let activeStorageKey = scopedStateStorageKey();

  function persistLocalState() {
    const nextKey = scopedStateStorageKey(state);
    if (activeStorageKey !== nextKey && GameStorage?.isDraftKey?.(activeStorageKey)) {
  ...
  function clearLocalState() {
    const keys = new Set([activeStorageKey, scopedStateStorageKey(state)]);
    for (const key of keys) if (key) localStorage.removeItem(key);
  }
  ...
  function loadState() {
    const routeGame = routeRemoteGameId();
    const keys = routeGame
      ? [scopedStateStorageKey()]
      : [scopedStateStorageKey(), LEGACY_STORAGE_KEY, ...LEGACY_STORAGE_KEYS];
    for (const key of [...new Set(keys)]) {
      try {
        ...
        stored.game.participation = stored.game.participation || {};
        stored.setup.roster = (stored.setup.roster || []).map(player => ({...player, availableToday:player.availableToday !== false}));
        ...
        activeStorageKey = scopedStateStorageKey(stored);
        localStorage.setItem(activeStorageKey, JSON.stringify(stored));
        return stored;
  ...
  function setSyncStatus(label, status = "local") {
    ...
    const gameBadge = $("gameSyncBadge");
    if (gameBadge) {
      const friendlyLabel = status === "offline"
        ? "Offline · saved here"
        : status === "error"
          ? "Sync issue"
          : status === "saving"
            ? "Syncing…"
            : ["Saved","Synced","Game reopened"].includes(label)
              ? "Synced"
              : label;
      gameBadge.textContent = friendlyLabel;
      gameBadge.dataset.state = status;
    }
  }


  function pilotReadinessState() {
    const rule = starterRule();
    const teamName = String(state.setup.teamName || "").trim();
    const opponentName = String(state.setup.opponentName || "").trim();
    const matchupReady = Boolean(teamName && opponentName && teamName.toLowerCase() !== opponentName.toLowerCase());
    const rosterCount = availableRoster().length;
    const rosterReady = rosterCount >= rule.total;
    const remoteReady = Boolean(state.game.remoteId);
    const scorerReady = Boolean(scorerControl?.canScore);
    const online = navigator.onLine !== false;
    const groupMeReady = Boolean(groupMeDestination?.enabled) && !state.game.messagesPaused && state.setup.messageFrequency !== "none";
    const canStart = matchupReady && rosterReady && remoteReady && scorerReady;
    let blocker = "";
    if (!matchupReady) blocker = "Confirm both teams before starting.";
    else if (!rosterReady) blocker = `Add at least ${rule.total} available players before starting.`;
    else if (!remoteReady) blocker = "Open this game from Game Day so WPI can protect the official game record.";
    else if (!scorerReady) blocker = "Scoring control is not ready on this device yet.";
    return {rule,teamName,opponentName,matchupReady,rosterCount,rosterReady,remoteReady,scorerReady,online,groupMeReady,canStart,blocker};
  }

  function setPreflightItem(id, stateName, detail) {
    const item = $(id);
    if (!item) return;
  ...
  function refreshPilotReadiness() {
    const readiness = pilotReadinessState();
    setPreflightItem("preflightMatchup", readiness.matchupReady ? "ready" : "block", readiness.matchupReady
      ? `${readiness.teamName} vs ${readiness.opponentName}`
      : "Confirm two different team names.");
    setPreflightItem("preflightRoster", readiness.rosterReady ? "ready" : "block", readiness.rosterReady
      ? `${readiness.rosterCount} available · ${readiness.rule.label}`
      : `${readiness.rosterCount} available · need ${readiness.rule.total}.`);
    setPreflightItem("preflightScorer", readiness.scorerReady ? "ready" : readiness.remoteReady ? "block" : "pending", readiness.scorerReady
      ? `${scorerControl?.activeDisplayName || "This device"} controls scoring.`
      : readiness.remoteReady ? "Scoring control is not ready on this device." : "Open from Game Day to confirm scoring control.");
    setPreflightItem("preflightConnection", readiness.online ? "ready" : "warn", readiness.online
      ? "Connected. WPI can sync saved plays."
      : "Offline. Keep scoring — plays stay on this device until connection returns.");
    setPreflightItem("preflightGroupMe", readiness.groupMeReady ? "ready" : "warn", readiness.groupMeReady
      ? `Score updates ready${groupMeDestination?.display_name ? ` · ${groupMeDestination.display_name}` : ""}.`
      : "Score updates are off. The game will still be recorded in WPI.");

    const button = $("startGameButton");
    if (button && state.game.status === "setup") {
      button.disabled = !readiness.canStart;
      button.textContent = "Start game";
      button.title = readiness.canStart ? "Choose starters and start scoring." : readiness.blocker;
    }
    const summary = $("preflightSummary");
    if (summary) {
      summary.dataset.state = readiness.canStart ? "ready" : "block";
      summary.textContent = readiness.canStart
        ? (readiness.online ? "Ready to start." : "Ready to start offline. WPI will sync when connection returns.")
        : readiness.blocker;
    }
    return readiness;
  }
  ...
  function markOffline() {
    ...
    if ($("connectionDetail")) $("connectionDetail").textContent = "Keep scoring. WPI will sync saved plays and GroupMe updates when connection returns.";
  }

  async function restoreConnectedContinuity({reason = "resume"} = {}) {
    refreshPilotReadiness();
    if (!backend || !workspace || navigator.onLine === false) {
      if (navigator.onLine === false) markOffline();
      return;
    }
    if (!state.game.remoteId) return;
    setSyncStatus(reason === "online" ? "Back online · syncing…" : "Checking game…", "saving");
    if ($("connectionDetail") && reason === "online") $("connectionDetail").textContent = "Connection restored. Syncing saved game activity and score updates.";
    try {
      await refreshScorerControl(state.game.remoteId);
  ...
  function syncGameScopedBackendAuthority() {
    if (!backend || !workspace) return;
    const permanentRole = workspace.role;
    const viewerRole = ["viewer","guest_viewer"].includes(permanentRole);
    if (viewerRole && scorerControl?.canScore) {
      // A Supporter may temporarily hold a valid game-scoped scorer session after
      // accepting a handoff. Keep the permanent membership role untouched, but
      // let the connected backend persist this game's events/deliveries as an
      // active game scorer. Database RLS still requires the active scorer session.
      backend.workspace = {
        ...(backend.workspace || workspace),
        role:"guest_scorer",
        scorerSessionId:scorerControl.activeSessionId || backend.workspace?.scorerSessionId || null,
        scorerDisplayName:scorerControl.activeDisplayName || backend.workspace?.scorerDisplayName || workspace.scorerDisplayName || null
      };
      return;
    }
    if (viewerRole && backend.workspace?.role === "guest_scorer") {
      backend.workspace = {...backend.workspace, role:permanentRole};
    }
  }

  function restoreSupporterDisabledControls() {
    document.querySelectorAll('[data-wpi-supporter-disabled="1"]').forEach(control => {
      control.disabled = false;
      delete control.dataset.wpiSupporterDisabled;
    });
  }
  ...
  function applyScorerControl(control) {
    ...
    syncGameScopedBackendAuthority();
    renderScorerControl();
    applyRoleAccess();
    refreshPilotReadiness();
  }
  ...
  async function pushRemoteState() {
    ...
    try {
      ...
      if (groupMeDestination?.enabled) {
        for (const message of state.game.messages || []) {
          if (message.status === "mock") {
  ...
  function effectivePlayerCap(player) {
    if (!player) return "";
    return String(player.gameCap ?? player.cap ?? "").trim();
  }

  function playerLabel(player) {
    if (!player) return "Team";
    const cap = effectivePlayerCap(player);
  ...
  function recalculateScore() {
    ...
    for (const event of activeEvents()) {
      if (event.type === "score_correction") {
        team = Number(event.correctedTeamScore || 0);
        opponent = Number(event.correctedOpponentScore || 0);
      } else {
        team = roundScore(team + (event.teamDelta || 0));
        opponent = roundScore(opponent + (event.opponentDelta || 0));
      }
      event.scoreAfter = {team:roundScore(team), opponent:roundScore(opponent)};
    }
  ...
  function participationEntry(playerId) {
    state.game.participation = state.game.participation || {};
    return state.game.participation[String(playerId)] || null;
  }

  function setParticipation(playerId, status, source = "manual") {
    if (!playerId) return;
    state.game.participation = state.game.participation || {};
    state.game.participation[String(playerId)] = {
      status:status === "dnp" ? "dnp" : "played",
      source,
      updatedAt:new Date().toISOString()
    };
  }

  function markPlayed(playerId, source = "event") {
    if (!playerId) return;
    const existing = participationEntry(playerId);
    if (existing?.status === "played") return;
    setParticipation(playerId,"played",source);
  }

  function unresolvedParticipationPlayers() {
    return availableRoster().filter(player => !["played","dnp"].includes(participationEntry(player.id)?.status));
  }

  function syncGameRosterParticipation() {
    state.game.participation = state.game.participation || {};
    for (const player of state.setup.roster || []) {
      if (player.availableToday === false) {
        const existing = participationEntry(player.id);
        if (!existing || existing.source === "game_roster") setParticipation(player.id,"dnp","game_roster");
      } else {
        const existing = participationEntry(player.id);
        if (existing?.status === "dnp" && existing.source === "game_roster") delete state.game.participation[String(player.id)];
      }
    }
  }
  ...
  function rosterFromForm() {
    ...
    return Array.from($("rosterList").querySelectorAll(".live-roster-row")).map(row => {
      ...
      return {
        ...(existing.get(id) || {}),
        id,
        cap: (existing.get(id) || {}).cap || "",
        gameCap: row.querySelector("[data-field='gameCap']").value.trim(),
        name: row.querySelector("[data-field='name']").value.trim(),
        availableToday: row.querySelector("[data-field='availableToday']")?.checked !== false
      };
    });
  }

  function availableRoster() {
    return state.setup.roster.filter(player => player.availableToday !== false && (String(effectivePlayerCap(player) || "").trim() || String(player.name || "").trim()));
  }
  ...
  function updateTeamRosterHeading() {
    ...
    if ($("rosterHelp")) $("rosterHelp").textContent = `Confirm who is available today. Game caps can change without changing the permanent roster; a player may be identified by name, cap, or both.`;
  }
  ...
  function renderRoster(focusPlayerId = null) {
    if (!state.setup.roster.length) state.setup.roster = [{id:uid("player"), cap:"", gameCap:"", name:"", createdByUserId:backend?.user?.id || null}];
    $("rosterList").innerHTML = state.setup.roster.map(player => `
      <div class="live-roster-row" data-player-id="${escapeHtml(player.id)}">
        <label>Game cap<input data-field="gameCap" type="text" inputmode="numeric" maxlength="3" value="${escapeHtml(effectivePlayerCap(player))}" placeholder="Optional"></label>
        <label>Player name<input data-field="name" type="text" maxlength="80" value="${escapeHtml(player.name || "")}" placeholder="Optional if cap is known"></label>
        <label class="wpi-game-roster-toggle"><input data-field="availableToday" type="checkbox" ${player.availableToday !== false ? "checked" : ""}><span>On today's game roster</span></label>
    ...
    $("rosterList").querySelectorAll("[data-remove-player]").forEach(button => button.addEventListener("click", () => {
      ...
      if (!state.setup.roster.length) state.setup.roster.push({id:uid("player"), cap:"", name:"", availableToday:true, createdByUserId:backend?.user?.id || null});
      saveState();
      renderRoster();
      renderPlayerOptions();
    }));
    ...
    if (focusPlayerId) requestAnimationFrame(() => {
      ...
      const input = row?.querySelector("[data-field='gameCap']");
  ...
  function addRosterPlayer() {
    state.setup.roster = rosterFromForm();
    const player = {id:uid("player"), cap:"", gameCap:"", name:"", availableToday:true, createdByUserId:backend?.user?.id || null};
    state.setup.roster.push(player);
    saveState();
    renderRoster(player.id);
    renderPlayerOptions();
    $("rosterStatus").textContent = "New player added. Enter a game cap, player name, or both.";
  }

  window.WPILiveGameCapAPI = {
    getGameId: () => state.game.remoteId || routeRemoteGameId(),
    getAssignments: () => Object.fromEntries((state.setup.roster || []).map(player => [String(player.id), String(effectivePlayerCap(player) || "").trim()])),
    applyAssignments(assignments = {}, {onlyEmpty=false} = {}) {
      const map = assignments || {};
      let changed = false;
      for (const player of state.setup.roster || []) {
        const key = String(player.id || "");
        const next = Object.prototype.hasOwnProperty.call(map,key) ? String(map[key] || "").trim() : null;
        if (next === null) continue;
        if (onlyEmpty && String(player.gameCap || "").trim()) continue;
        if (String(player.gameCap || "").trim() !== next) { player.gameCap = next; changed = true; }
      }
      if (changed) { saveState(); renderRoster(); renderPlayerOptions(); renderAll(); }
      return changed;
    },
    effectiveRoster: () => (state.setup.roster || []).map(player => ({id:player.id,remoteId:player.remoteId || null,name:player.name || "",defaultCap:player.cap || "",gameCap:effectivePlayerCap(player)}))
  };

  function renderStarterRule() {
    const rule = starterRule();
    if ($("launchRuleText")) $("launchRuleText").textContent = `${state.setup.ageGroup}: ${rule.label}. Your saved lineup will be preselected.`;
    refreshPilotReadiness();
  }
  ...
  function renderPlayerOptions() {
    const sorted = [...availableRoster()].sort((a,b) => {
      const capA = Number.parseInt(effectivePlayerCap(a),10);
      const capB = Number.parseInt(effectivePlayerCap(b),10);
      if (Number.isFinite(capA) && Number.isFinite(capB)) return capA - capB;
      return String(effectivePlayerCap(a)).localeCompare(String(effectivePlayerCap(b)));
    });
  ...
  function formatMessage(event) {
    ...
    const when = event.phase === "shootout" || event.type.startsWith("shootout_")
      ...
      : event.timePrecision === "inherited" ? `${quarterText(event.quarter)}` : `${quarterText(event.quarter)} · ${event.timeRemaining}`;
  ...
  function openLineupDialog(quarter) {
    syncSetupFromForm();
    if (Number(quarter) === 1 && state.game.status === "setup") {
      const readiness = refreshPilotReadiness();
      if (!readiness.canStart) {
        if (readiness.blocker) alert(readiness.blocker);
        return;
      }
    }
    const rule = starterRule();
    ...
    if ($("undoQuarterEndButton")) $("undoQuarterEndButton").hidden = !locked;
    $("saveLineupButton").textContent = pendingLineupQuarter === 1 ? "Confirm starters & begin" : `Start ${quarterText(pendingLineupQuarter)}`;
  ...
  function baseEvent(type, overrides = {}) {
    return {
      ...
      timePrecision:overrides.timePrecision || "exact",
      previousTimeRemaining:overrides.previousTimeRemaining || null,
      note:overrides.note || "",
  ...
  function commitEvent(event, type) {
    ...
    if (event.playerId) markPlayed(event.playerId,"event");
    if (event.secondaryPlayerId) markPlayed(event.secondaryPlayerId,"event");
    recalculateScore();
  ...
  function addSystemEvent(eventType, overrides = {}) {
    ...
    const event = baseEvent(type, overrides);
  ...
  function saveLineup() {
    ...
    choices.forEach(id => markPlayed(id,"lineup"));
  ...
  function endQuarter() {
    ...
    if (!confirm(`End ${quarterText(current)}?\n\nYou can undo this immediately from the next screen if you tapped it early.`)) return;

    if (document.activeElement instanceof HTMLElement) document.activeElement.blur();
    resetEventEntry({preserveClock:true});
    const previousTimeRemaining = clockText();
    ...
    if (!last) addSystemEvent("quarter_end", {quarter:current, timeRemaining:"0:00", previousTimeRemaining});
  ...
  function undoEndQuarter() {
    if (!scoringActionAllowed()) return;
    const current = Number(state.game.quarter);
    const endEvent = [...state.game.events].reverse().find(event => event.status !== "voided" && event.type === "quarter_end" && Number(event.quarter) === current);
    if (!endEvent) return;
    endEvent.status = "voided";
    const endMessage = (state.game.messages || []).find(message => message.eventId === endEvent.id);
    if (!endMessage || endMessage.status !== "sent") state.game.messages = (state.game.messages || []).filter(message => message.eventId !== endEvent.id);
    state.game.pendingQuarter = null;
    state.game.status = "live";
    const restored = parseClock(endEvent.previousTimeRemaining || "") || {minutes:state.setup.quarterLength,seconds:0,text:`${state.setup.quarterLength}:00`};
    state.game.clockMinutes = restored.minutes;
    state.game.clockSeconds = restored.seconds;
    $("clockTime").value = restored.text;
    recalculateScore();
    for (const dialog of [$("lineupDialog"),$("postPeriodDialog")]) if (dialog?.open) dialog.close();
    saveState();
    renderAll();
    resetEventEntry({preserveClock:true});
    setSyncStatus("Quarter restored · syncing…","saving");
    scheduleRemoteSync(25);
  }
  ...
  function formTimePrecision() {
    const value = $("eventForm")?.dataset?.wpiTimePrecision;
    return value === "inherited" ? "inherited" : "exact";
  }
  ...
  function recordSelectedEvent(event) {
    ...
    const timePrecision = formTimePrecision();
    const gameEvent = baseEvent(type, {playerId, secondaryPlayerId:assistId, note, timePrecision});
    // Submit-play workflow: record immediately. Mistakes are handled through Undo or score correction.
    commitEvent(gameEvent, type);

    $("eventValidationMessage").textContent = gameEvent.timePrecision === "inherited" ? `${type.label} recorded. Clock unchanged.` : `${type.label} submitted at ${gameEvent.timeRemaining}.`;
    if ($("eventForm")) delete $("eventForm").dataset.wpiTimePrecision;
    $("eventType").value = "";
    $("primaryPlayer").value = "";
    $("assistPlayer").value = "";
    $("eventNote").value = "";
  ...
  function editableLastEvent() {
    return [...state.game.events].reverse().find(event => event.status !== "voided" && !SYSTEM_EVENT_TYPES.has(event.type) && !event.type.startsWith("shootout_")) || null;
  }

  function createCorrectionAudit(text) {
    const type = {id:"stat_correction", label:"Correction", category:"game", priority:"major"};
    const correction = baseEvent(type, {note:text, timePrecision:"inherited"});
    state.game.events.push(correction);
    recalculateScore();
    correction.scoreAfter = {team:state.game.teamScore, opponent:state.game.opponentScore};
    createMessage(correction,type,{text:`Correction · ${quarterText(correction.quarter)}\n${text}\n${state.setup.teamName} ${displayScore(state.game.teamScore)}, ${state.setup.opponentName} ${displayScore(state.game.opponentScore)}`});
    return correction;
  }

  function renderParticipationReview() {
    const unresolved = unresolvedParticipationPlayers();
    const list = $("participationReviewList");
    if (!list) return unresolved;
    list.innerHTML = unresolved.length ? unresolved.map(player => {
      const current = participationEntry(player.id)?.status || "";
      return `<div class="wpi-participation-row" data-participation-player="${escapeHtml(player.id)}"><div><strong>${escapeHtml(playerLabel(player))}</strong><small>No recorded play or saved starting lineup confirmed participation.</small></div><div class="wpi-participation-actions"><button type="button" data-participation-status="played" class="${current === "played" ? "is-active" : ""}">Played</button><button type="button" data-participation-status="dnp" class="${current === "dnp" ? "is-active" : ""}">DNP</button></div></div>`;
    }).join("") : '<p class="live-empty-state">Participation is confirmed for every player on today\'s game roster.</p>';
    $("participationReviewMessage").textContent = unresolved.length ? `${unresolved.length} player${unresolved.length===1?"":"s"} need confirmation.` : "Participation complete.";
    list.querySelectorAll("[data-participation-status]").forEach(button => button.addEventListener("click", () => {
      const row = button.closest("[data-participation-player]");
      if (!row) return;
      setParticipation(row.dataset.participationPlayer,button.dataset.participationStatus,"final_review");
      saveState();
      renderParticipationReview();
    }));
    return unresolved;
  }

  function openParticipationReview() {
    syncGameRosterParticipation();
    renderParticipationReview();
    $("participationReviewDialog")?.showModal();
  }

  function markAllUnresolvedPlayed() {
    for (const player of unresolvedParticipationPlayers()) setParticipation(player.id,"played","final_review_bulk");
    saveState();
    renderParticipationReview();
  }

  function eventTypeOptions(selected) {
    return EVENT_TYPES.map(type => `<option value="${escapeHtml(type.id)}"${type.id===selected?" selected":""}>${escapeHtml(type.dropdownLabel || type.label)}</option>`).join("");
  }

  function playerOptions(selected, emptyLabel = "Select player") {
    const rows = [...availableRoster()].sort((a,b)=>String(effectivePlayerCap(a)).localeCompare(String(effectivePlayerCap(b)),undefined,{numeric:true}));
    return `<option value="">${escapeHtml(emptyLabel)}</option>${rows.map(player => `<option value="${escapeHtml(player.id)}"${player.id===selected?" selected":""}>${escapeHtml(playerLabel(player))}</option>`).join("")}`;
  }

  let editingEventId = null;
  function updateEditPlayFields() {
    const type = EVENT_MAP.get($("editPlayType")?.value || "");
    if (!type) return;
    $("editPlayPlayerLabel").hidden = !type.requiresPlayer;
    $("editPlayAssistLabel").hidden = !type.allowsAssist;
    $("editPlayTimeLabel").hidden = !EXACT_TIME_EVENT_TYPES.has(type.id);
    if (!type.allowsAssist) $("editPlayAssist").value = "";
  }

  function openEditLastPlay(eventId = null) {
    const event = eventId ? state.game.events.find(item => item.id===eventId && item.status!=="voided") : editableLastEvent();
    if (!event || !EVENT_MAP.has(event.type)) return;
    editingEventId = event.id;
    $("editPlayType").innerHTML = eventTypeOptions(event.type);
    $("editPlayPlayer").innerHTML = playerOptions(event.playerId || null);
    $("editPlayAssist").innerHTML = playerOptions(event.secondaryPlayerId || null,"Unassisted");
    $("editPlayTime").value = event.timeRemaining || clockText();
    $("editPlayNote").value = event.note || "";
    $("editPlayMessage").textContent = "";
    updateEditPlayFields();
    $("editLastPlayDialog")?.showModal();
  }

  function saveEditedPlay() {
    if (!scoringActionAllowed()) return;
    const event = state.game.events.find(item => item.id===editingEventId && item.status!=="voided");
    const type = EVENT_MAP.get($("editPlayType")?.value || "");
    if (!event || !type) return;
    const playerId = type.requiresPlayer ? $("editPlayPlayer").value : "";
    const assistId = type.allowsAssist ? $("editPlayAssist").value : "";
    if (type.requiresPlayer && !playerId) { $("editPlayMessage").textContent="Choose the player involved."; return; }
    if (playerId && assistId===playerId) { $("editPlayMessage").textContent="Scorer and assist cannot be the same player."; return; }
    let parsed = null;
    if (EXACT_TIME_EVENT_TYPES.has(type.id)) {
      parsed = parseClock($("editPlayTime").value);
      if (!parsed) { $("editPlayMessage").textContent="Enter a valid time such as 5:42."; return; }
    }
    const oldLabel = event.label;
    const oldPlayer = playerById(event.playerId);
    const originalMessage = (state.game.messages || []).find(message => message.eventId===event.id);
    const wasSent = originalMessage?.status === "sent";
    event.type=type.id; event.label=type.label; event.category=type.category;
    event.playerId=playerId || null; event.secondaryPlayerId=assistId || null;
    event.note=$("editPlayNote").value.trim();
    event.teamDelta=Number(type.teamDelta||0); event.opponentDelta=Number(type.opponentDelta||0);
    event.teamShotDelta=Number(type.teamShotDelta||0); event.opponentShotDelta=Number(type.opponentShotDelta||0);
    event.opponentFieldBlockDelta=Number(type.opponentFieldBlockDelta||0); event.opponentSaveDelta=Number(type.opponentSaveDelta||0);
    event.saveDelta=Number(type.saveDelta||0); event.fieldBlockDelta=Number(type.fieldBlockDelta||0);
    event.timePrecision=EXACT_TIME_EVENT_TYPES.has(type.id)?"exact":"inherited";
    if (parsed) event.timeRemaining=parsed.text;
    if (event.playerId) markPlayed(event.playerId,"event_edit");
    if (event.secondaryPlayerId) markPlayed(event.secondaryPlayerId,"event_edit");
    recalculateScore();
    if (originalMessage && !wasSent) {
      originalMessage.text=formatMessage(event);
      if (backend && groupMeDestination?.enabled) originalMessage.status="pending";
      originalMessage.lastError=""; originalMessage.nextRetryAt=null;
    }
    if (wasSent) {
      const oldWho = oldPlayer ? ` — ${playerLabel(oldPlayer)}` : "";
      const newPlayer = playerById(event.playerId);
      const newWho = newPlayer ? ` — ${playerLabel(newPlayer)}` : "";
      createCorrectionAudit(`${oldLabel}${oldWho} changed to ${type.label}${newWho}.`);
    }
    state.analyticsSnapshot=null;
    saveState();
    $("editLastPlayDialog")?.close();
    renderAll();
    scheduleRemoteSync(25);
  }
  ...
  function undoLastEvent() {
    if (!scoringActionAllowed()) return;
    const last = editableLastEvent();
    if (!last) return;
    const message = (state.game.messages || []).find(item => item.eventId===last.id);
    const sent = message?.status === "sent";
    const label = `${last.label}${last.playerId && playerById(last.playerId) ? ` — ${playerLabel(playerById(last.playerId))}` : ""}`;
    last.status = "voided";
    if (!sent) state.game.messages = (state.game.messages || []).filter(item => item.eventId !== last.id);
    recalculateScore();
    if (sent) createCorrectionAudit(`${label} was removed from the official WPI record.`);
    state.analyticsSnapshot=null;
    saveState();
    renderAll();
    scheduleRemoteSync(25);
  ...
  function renderLastUpdate() {
    const last = [...activeEvents()].reverse().find(event => !SYSTEM_EVENT_TYPES.has(event.type));
    if (!last) {
      $("lastUpdateCard").hidden = true;
      return;
    }
    const player = playerById(last.playerId);
    const period = last.phase === "shootout" ? `SO R${last.shootoutRound}` : last.timePrecision === "inherited" ? `${quarterText(last.quarter)}` : `${quarterText(last.quarter)} ${last.timeRemaining}`;
  ...
  function buildRecap(stats) {
    const team = state.setup.teamName || "The team";
    const opponent = state.setup.opponentName || "the opponent";
    const teamScore = Number(state.game.teamScore || 0);
    const opponentScore = Number(state.game.opponentScore || 0);
    const margin = Math.abs(teamScore - opponentScore);
    const won = teamScore > opponentScore;
    const lost = teamScore < opponentScore;
    const tied = teamScore === opponentScore;
    const saves = Number(stats.counts.save || 0);
    const blocks = Number(stats.counts.field_block || 0);
    const steals = Number(stats.counts.steal || 0);
    const defense = saves + blocks + steals;
    const goals = Number(stats.counts.goal || 0);
    const scoringEvents = activeEvents().filter(event => ["goal","opponent_goal"].includes(event.type));
    let teamRun = 0, opponentRun = 0, bestTeamRun = 0, bestOpponentRun = 0;
    for (const event of scoringEvents) {
      if (event.type === "goal") { teamRun += 1; opponentRun = 0; bestTeamRun = Math.max(bestTeamRun,teamRun); }
      else { opponentRun += 1; teamRun = 0; bestOpponentRun = Math.max(bestOpponentRun,opponentRun); }
    }
    let opening;
    if (tied) opening = `${team} and ${opponent} finished level at ${displayScore(teamScore)}–${displayScore(opponentScore)} in a game that stayed unsettled to the end.`;
    else if (won && margin >= 6) opening = `${team} took care of business against ${opponent}, pulling away for a convincing ${displayScore(teamScore)}–${displayScore(opponentScore)} win.`;
    else if (won && margin <= 2) opening = `${team} had to earn every bit of this one, edging ${opponent} ${displayScore(teamScore)}–${displayScore(opponentScore)} in a game that went down to the wire.`;
    else if (won) opening = `${team} found enough separation to beat ${opponent} ${displayScore(teamScore)}–${displayScore(opponentScore)}.`;
    else if (lost && margin >= 6) opening = `${team} had a rough one against ${opponent}, falling ${displayScore(teamScore)}–${displayScore(opponentScore)} in a game that got away from them.`;
    else if (lost && margin <= 2) opening = `${team} came up just short against ${opponent}, dropping a tight ${displayScore(teamScore)}–${displayScore(opponentScore)} game.`;
    else opening = `${team} fell ${displayScore(teamScore)}–${displayScore(opponentScore)} to ${opponent}.`;

    const themes = [];
    if (blocks >= 5 && steals >= 4) themes.push(`The defense was tighter than Fort Knox for long stretches, piling up ${blocks} blocks and ${steals} steals.`);
    else if (defense >= 10) themes.push(`Defensively, ${team} stayed busy with ${saves} saves, ${blocks} blocks and ${steals} steals.`);
    else if (saves >= 6) themes.push(`${team} got plenty of help in goal, recording ${saves} saves.`);
    if (goals >= 12) themes.push(`The offense was humming with ${goals} recorded goals.`);
    if (bestTeamRun >= 4) themes.push(`${team} created real separation with a ${bestTeamRun}-goal unanswered run.`);
    if (bestOpponentRun >= 4 && lost) themes.push(`${opponent} did the biggest damage with a ${bestOpponentRun}-goal unanswered run that ${team} could not fully erase.`);

    const players = [...stats.players.values()].map(item => ({
      label:playerLabel(item.player),
      goals:Number(item.counts.goal || 0),
      assists:Number(item.assists || 0),
      saves:Number(item.counts.save || 0),
      steals:Number(item.counts.steal || 0),
      blocks:Number(item.counts.field_block || 0)
    }));
    const highlights = [];
    const goalLeader = players.slice().sort((a,b)=>b.goals-a.goals)[0];
    if (goalLeader?.goals >= 2) highlights.push(`${goalLeader.label} led the scoring with ${goalLeader.goals} goals${goalLeader.assists ? ` and added ${goalLeader.assists} assist${goalLeader.assists===1?"":"s"}` : ""}.`);
    const saveLeader = players.slice().sort((a,b)=>b.saves-a.saves)[0];
    if (saveLeader?.saves >= 3) highlights.push(`${saveLeader.label} stood tall with ${saveLeader.saves} saves.`);
    const stealLeader = players.slice().sort((a,b)=>b.steals-a.steals)[0];
    if (stealLeader?.steals >= 2 && stealLeader?.label !== goalLeader?.label) highlights.push(`${stealLeader.label} created problems defensively with ${stealLeader.steals} steals.`);
    const blockLeader = players.slice().sort((a,b)=>b.blocks-a.blocks)[0];
    if (blockLeader?.blocks >= 2 && !highlights.some(text=>text.startsWith(blockLeader.label))) highlights.push(`${blockLeader.label} chipped in ${blockLeader.blocks} field blocks.`);

    const closing = lost && margin >= 6 ? `${team} will have plenty to clean up before the next one, but the individual bright spots are there in the recorded stats.` : "";
    const shootoutText = state.game.shootout?.active ? ` The shootout finished ${state.game.shootout.teamGoals}–${state.game.shootout.opponentGoals}.` : "";
    return [opening + shootoutText, ...themes.slice(0,2), ...highlights.slice(0,3), closing].filter(Boolean).join(" ") + " Statistics are unofficial and reflect scorer-entered actions.";
  }

  function buildGroupMeSummary(stats) {
    const assists = [...stats.players.values()].reduce((sum,item) => sum + item.assists, 0);
    const gameStory = buildRecap(stats).replace(/\s*Statistics are unofficial and reflect scorer-entered actions\.$/, "");
    const lines = [
      `${state.setup.teamName} ${displayScore(state.game.teamScore)} — ${state.setup.opponentName} ${displayScore(state.game.opponentScore)}`,
      "",
      "GAME STORY",
    ...
    lines.push("", "Full player stats are available in WPI.", "Unofficial stats reflect scorer-entered actions.");
    return lines.join("\n");
  }

  function buildTeamStatsCardDataUrl(stats) {
    const canvas = document.createElement("canvas");
    canvas.width = 1200;
    canvas.height = 675;
    const ctx = canvas.getContext("2d");
    if (!ctx) return null;
    const team = state.setup.teamName || "WPI Live";
    const opponent = state.setup.opponentName || "Opponent";
    const assists = [...stats.players.values()].reduce((sum,item) => sum + item.assists, 0);
    const shooting = stats.teamShots ? Math.round(((stats.counts.goal || 0) / stats.teamShots) * 100) : 0;
    const metrics = [
      ["GOALS", stats.counts.goal || 0], ["SHOTS", stats.teamShots], ["SHOOTING", `${shooting}%`],
      ["SAVES", stats.counts.save || 0], ["STEALS", stats.counts.steal || 0], ["TURNOVERS", stats.counts.turnover || 0],
      ["FIELD BLOCKS", stats.counts.field_block || 0], ["ASSISTS", assists], ["EXCL. DRAWN", stats.counts.exclusion_drawn || 0],
      ["EXCL. COMMITTED", stats.counts.exclusion_committed || 0], ["5M DRAWN", stats.counts.five_meter_drawn || 0], ["5M COMMITTED", stats.counts.five_meter_committed || 0]
    ];
    ctx.fillStyle="#f8fafc"; ctx.fillRect(0,0,canvas.width,canvas.height);
    ctx.fillStyle="#0f2742"; ctx.font="700 26px Arial"; ctx.fillText("WPI LIVE · TEAM STATS",44,52);
    ctx.font="700 38px Arial"; ctx.fillText(`${team} ${displayScore(state.game.teamScore)} — ${opponent} ${displayScore(state.game.opponentScore)}`,44,104);
    ctx.fillStyle="#5f718b"; ctx.font="600 20px Arial"; ctx.fillText("Unofficial scorer-entered statistics",44,139);
    const cols=4, gap=16, left=44, top=174, usable=1112;
    const boxW=(usable-gap*(cols-1))/cols, boxH=137;
    metrics.forEach(([label,value],i)=>{
      const col=i%cols,row=Math.floor(i/cols),x=left+col*(boxW+gap),y=top+row*(boxH+gap);
      ctx.fillStyle="#eef2f7"; ctx.fillRect(x,y,boxW,boxH);
      ctx.fillStyle="#081a33"; ctx.font="700 38px Arial"; ctx.fillText(String(value),x+22,y+54);
      ctx.fillStyle="#5f718b"; ctx.font="700 18px Arial"; ctx.fillText(String(label),x+22,y+92);
    });
    ctx.fillStyle="#5f718b"; ctx.font="600 16px Arial"; ctx.fillText("Full player stats are available in WPI.",44,650);
    return canvas.toDataURL("image/png");
  }

  async function prepareFinalWhistleStatsImage(stats) {
    state.game.groupMeStatsImageUrl = null;
    if (!backend || !groupMeDestination?.enabled || !state.game.remoteId) return null;
    try {
      const dataUrl = buildTeamStatsCardDataUrl(stats);
      if (!dataUrl) return null;
      const result = await backend.uploadGroupMeStatsCard(state.game.remoteId,dataUrl);
      state.game.groupMeStatsImageUrl = result?.url || null;
      persistLocalState();
      return state.game.groupMeStatsImageUrl;
    } catch (error) {
      console.warn("WPI Final Whistle stats image upload failed", error);
      return null;
    }
  }
  ...
  function ensureAutomaticGameSummary() {
    ...
    messages.forEach((messageText, index) => {
      ...
      if (part === 1 && state.game.groupMeStatsImageUrl) event.groupMeImageUrl = state.game.groupMeStatsImageUrl;
      state.game.events.push(event);
      event.scoreAfter = {team:state.game.teamScore, opponent:state.game.opponentScore};
      createMessage(event, type, {
        text:messageText,
  ...
  function renderAll() {
    ...
    if (state.game.status === "setup") {
      ...
      document.body.classList.remove("is-live-game");
      $("setupPanel").hidden = false;
      $("gamePreflight").hidden = false;
      $("launchRow").hidden = false;
      $("liveConsole").hidden = true;
    } else if (editingSetup) {
      document.body.classList.remove("is-live-game");
      $("setupPanel").hidden = false;
      $("gamePreflight").hidden = true;
  ...
  async function endGame({skipConfirm = false, skipParticipationReview = false} = {}) {
    if (!scoringActionAllowed()) return;
    syncGameRosterParticipation();
    if (!skipParticipationReview && unresolvedParticipationPlayers().length) {
      openParticipationReview();
      return;
    }
    ...
    try {
      const finalStats = aggregate();
      await prepareFinalWhistleStatsImage(finalStats);
      if (state.game.phase !== "shootout") {
      ...
      if (backend && workspace) {
        ...
        const summaryIds = new Set(automaticSummaryEvents().map(event => event.id));
        await deliverPendingMessages(result, summaryIds);
        await refreshDeliveryStatuses(result.remoteGameId);
        const summaryMessages = (state.game.messages || []).filter(message => summaryIds.has(message.eventId));
        const allSummarySent = summaryMessages.length > 0 && summaryMessages.every(message => ["sent","suppressed"].includes(message.status));
        setSyncStatus(allSummarySent ? "Final saved · Summary sent" : "Final saved · Summary queued", "saved");
      }
      await refreshReopenEligibility();
  ...
  let reopenEligibilityRequestId = 0;

  async function refreshReopenEligibility() {
    const button = $("reopenGameButton");
    if (!button) return false;
    const requestId = ++reopenEligibilityRequestId;
    button.hidden = true;

    if (!backend || !workspace || !state.game.remoteId || state.game.status !== "ended") return false;

    try {
      const {data,error} = await backend.client.rpc("live_reopen_game_eligibility_v1", {
        target_game_id:state.game.remoteId
      });
      if (error) throw error;
      if (requestId !== reopenEligibilityRequestId || state.game.status !== "ended") return false;
      const allowed = Boolean(data?.canReopen);
      button.hidden = !allowed;
      button.disabled = false;
      button.textContent = "Reopen game";
      if (allowed && data?.expiresAt) button.title = "Available during the scorer recovery window.";
      else button.removeAttribute("title");
      return allowed;
    } catch (_) {
      if (requestId === reopenEligibilityRequestId) button.hidden = true;
      return false;
    }
  }
  ...
  async function reopenGame() {
    ...
    try {
      ...
      // A premature final is reopened into the same active period/clock. Clear
      // any stale transition marker so the scorer does not need to touch the
      // quarter selector merely to reactivate play entry.
      if (state.game.phase !== "shootout") state.game.pendingQuarter = null;
      voidAccidentalFinalOutput();
      restoreSupporterDisabledControls();
    ...
    // Rebuild every poolside input from the reopened game state immediately.
    // This makes scoring available without a synthetic quarter-change event.
    restoreSupporterDisabledControls();
    renderScoreboard();
    resetEventEntry({preserveClock:true});
    applyRoleAccess();
  ...
  async function authorize() {
    ...
    try {
      ...
      if (remoteGameId) {
        ...
        if (preserved.length) {
          state.setup.roster = preserved.map(player => ({...player, availableToday:true}));
  ...
  function applyRoleAccess() {
    ...
    if (reopenButton) {
      ...
      if (state.game.status === "ended") void refreshReopenEligibility();
    }
    ...
    if (isViewer) {
      ...
      const allowedButtons = new Set(["signOutButton", "gameSignOutButton", "dashboardTopButton", "gameDashboardButton", "summaryDashboardButton", "downloadLogButton", "enterScorerCodeInlineButton"]);
      document.querySelectorAll("#liveSandboxApp input, #liveSandboxApp select, #liveSandboxApp textarea, #liveSandboxApp button").forEach(control => {
        ...
        if (!control.disabled) control.dataset.wpiSupporterDisabled = "1";
      ...
      // Supporter read-only mode intentionally disables the live controls. If that
      // same account later accepts a scorer handoff (or a reopened game restores
      // its scorer session), reverse only the controls that Supporter mode disabled.
      restoreSupporterDisabledControls();
  ...
  function dashboardUrl() {
    const url = new URL("live-dashboard.html", window.location.href);
  ...
  function returnToDashboard() {
    const gameOpen = !["ended","final","cancelled"].includes(state.game.status);
    if (gameOpen && scorerControl?.canScore) {
      const leave = confirm("Return to the dashboard?\n\nThis game will stay active and can be reopened from Game-Day Hub.");
      if (!leave) return;
    }
  ...
  function bindEvents() {
    ["teamName","opponentName","gameDateTime","gameVenue","ageGroup","quarterLength","groupMeName","messageFrequency","visibility"].forEach(id => $(id).addEventListener("change", () => { syncSetupFromForm(); refreshPilotReadiness(); }));
    ["teamName","opponentName"].forEach(id => $(id).addEventListener("input", () => { syncSetupFromForm(); refreshPilotReadiness(); }));
    ...
    $("editLastInlineButton")?.addEventListener("click", () => openEditLastPlay());
    $("editPlayType")?.addEventListener("change", updateEditPlayFields);
    $("saveEditedPlayButton")?.addEventListener("click", saveEditedPlay);
    $("markAllParticipationPlayedButton")?.addEventListener("click", markAllUnresolvedPlayed);
    $("confirmParticipationAndEndButton")?.addEventListener("click", () => {
      if (unresolvedParticipationPlayers().length) {
        $("participationReviewMessage").textContent = "Mark each remaining player as Played or DNP before Final Whistle.";
        return;
      }
      $("participationReviewDialog")?.close();
      void endGame({skipConfirm:true,skipParticipationReview:true});
    });
    $("undoQuarterEndButton")?.addEventListener("click", undoEndQuarter);
    $("undoPostPeriodEndButton")?.addEventListener("click", undoEndQuarter);
    ...
    window.addEventListener("online", () => void restoreConnectedContinuity({reason:"online"}));
    window.addEventListener("pageshow", () => void restoreConnectedContinuity({reason:"pageshow"}));
    document.addEventListener("visibilitychange", () => {
      if (!document.hidden) void restoreConnectedContinuity({reason:"resume"});
    });
    $("dashboardTopButton")?.addEventListener("click", returnToDashboard);
    $("gameDashboardButton")?.addEventListener("click", returnToDashboard);
    $("summaryDashboardButton")?.addEventListener("click", returnToDashboard);
  ...
  async function init() {
    ...
    if (repairedLegacySummary) persistLocalState();
    ...
    applyRoleAccess();
    refreshPilotReadiness();
    maybeAutoLaunchFromDashboard();
    if (navigator.onLine === false) markOffline();
    if (backend && workspace) {
      const hasRecoverableDelivery = (state.game.messages || []).some(message => deliveryIsDue(message));
      const hasGameScopedScoringAuthority = Boolean(scorerControl?.canScore);
      if ((workspace.role !== "viewer" || hasGameScopedScoringAuthority) && (!state.game.remoteId || hasRecoverableDelivery)) {
        scheduleRemoteSync(150);
      }
    }
  }
