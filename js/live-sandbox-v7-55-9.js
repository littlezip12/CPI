/* WPI 7.55.9 — manual-controls and auto-open activity flow.
...
(() => {
  ...
  const RELEASE = "7.55.9";
  const STORAGE_KEY = "wpi-live-sandbox-v7-55-9";
  const LEGACY_STORAGE_KEYS = ["wpi-live-sandbox-v7-55-8", "wpi-live-sandbox-v7-55-6", "wpi-live-sandbox-v7-55-5", "wpi-live-sandbox-v7-55-4", "wpi-live-sandbox-v7-55-3", "wpi-live-sandbox-v7-55-2"];
  const AUTH_KEY = "wpi-live-auth-v7-55-9";
  const LEGACY_AUTH_KEYS = ["wpi-live-auth-v7-55-8", "wpi-live-auth-v7-55-6", "wpi-live-auth-v7-55-5", "wpi-live-auth-v7-55-4", "wpi-live-auth-v7-55-3"];
  ...
  function rosterFromForm() {
    return Array.from($("rosterList").querySelectorAll(".live-roster-row")).map(row => ({
      id: row.dataset.playerId || uid("player"),
  ...
  function addRosterPlayer() {
    state.setup.roster = rosterFromForm();
    const player = {id:uid("player"), cap:"", name:""};
  ...
  function endQuarter() {
    if (state.game.pendingQuarter) return;
  ...
  function renderScoreboard() {
    ...
    $("endQuarterButton").disabled = Boolean(state.game.pendingQuarter) || state.game.status !== "live";
