/* WPI 7.56.8 — connected live backend with reliable GroupMe bot/topic delivery.
 * Browser-safe publishable keys only. Secret keys, GroupMe bot IDs, and GroupMe access tokens belong in Supabase Edge Function secrets.
 */
(() => {
  "use strict";
  ...
  class WPILiveBackend {
    ...
    static async connect(config = {}) {
      ...
      const client = module.createClient(config.supabaseUrl, config.supabasePublishableKey, {
        ...
        global: { headers: { "x-wpi-live-release": config.release || "7.56.8" } }
      });
    ...
    async loadGroupMeDestination(teamId = this.workspace?.teamId) {
      ...
      if (String(this.workspace?.role || "").startsWith("guest_")) {
        const { data, error } = await this.client
          ...
          .select("id,team_id,display_name,delivery_mode,groupme_group_id,groupme_group_name,groupme_topic_id,groupme_topic_name,enabled,last_tested_at,last_test_status,last_test_error,updated_at")
          .eq("team_id", teamId)
          .eq("provider", "groupme")
          .maybeSingle();
        if (error) throw error;
      ...
      this.destination = data ? {
        ...
        delivery_mode: data.deliveryMode || "bot",
    ...
    async saveGroupMeDestination(teamId, displayName, secretName, enabled = true, options = {}) {
      const mode = options.mode === "topic" ? "topic" : "bot";
      const { data, error } = await this.client.rpc("live_upsert_groupme_destination_v2", {
        target_team_id: teamId,
        destination_name: displayName,
        destination_secret_name: secretName || null,
        destination_enabled: Boolean(enabled),
        destination_mode: mode,
        destination_group_id: mode === "topic" ? (options.groupId || null) : null,
        destination_group_name: mode === "topic" ? (options.groupName || null) : null,
        destination_topic_id: mode === "topic" ? (options.topicId || null) : null,
        destination_topic_name: mode === "topic" ? (options.topicName || null) : null
      });
      if (error) throw error;
      this.destination = {
        id: data.id,
        ...
        display_name: data.displayName,
        secret_name: data.secretName || null,
        delivery_mode: data.deliveryMode || mode,
        groupme_group_id: data.groupId || null,
        groupme_group_name: data.groupName || null,
        groupme_topic_id: data.topicId || null,
        groupme_topic_name: data.topicName || null,
    ...
    async invokeGroupMeAdminAction(body, fallbackMessage) {
      const { data, error } = await this.client.functions.invoke("groupme-post-v7-64-11", { body });
      if (error) {
    ...
    async discoverGroupMeGroups(teamId, secretName = null) {
      if (!teamId) throw new Error("Team is required.");
      const data = await this.invokeGroupMeAdminAction({
        action: "discover_groups",
        team_id: teamId,
        secret_name: secretName || null
      }, "GroupMe groups could not be loaded");
      return Array.isArray(data?.groups) ? data.groups : [];
    }

    async discoverGroupMeTopics(teamId, groupId, secretName = null) {
      if (!teamId || !groupId) throw new Error("Choose a GroupMe group first.");
      const data = await this.invokeGroupMeAdminAction({
        action: "discover_topics",
        team_id: teamId,
        group_id: groupId,
        secret_name: secretName || null
      }, "GroupMe topics could not be loaded");
      return Array.isArray(data?.topics) ? data.topics : [];
    }

    async testGroupMeDestination(destinationId, text) {
      const { data, error } = await this.client.functions.invoke("groupme-post-v7-64-11", {
    ...
    async uploadGroupMeStatsCard(gameId, dataUrl) {
      const { data, error } = await this.client.functions.invoke("groupme-post-v7-64-11", {
        body: { action:"upload_stats_card", game_id:gameId, data_url:dataUrl }
      });
      if (error) {
        let message = error.message || "Team stats image upload failed";
    ...
    async acceptScorerHandoff({ token = null, code = null, gameId = null, displayName } = {}) {
      ...
      if (data?.gameId) {
        this.workspace = {
          ...(this.workspace || {}),
          teamId: data.teamId || this.workspace?.teamId || null,
          teamName: data.teamName || this.workspace?.teamName || null,
          rosterId: data.rosterId || this.workspace?.rosterId || null,
          role: data.role || "scorer",
          gameId: data.gameId,
          scorerSessionId: data.sessionId || null,
          scorerDisplayName: data.displayName || displayName || null
        };
      }
      return data;
    }
    ...
    async syncState(state) {
      ...
      if (canManageTeam) {
        rosterRows = (state.setup?.roster || [])
          ...
          .map((player, index) => ({
            ...
            cap_number: String(player.cap || "").trim() || null,
            display_name: String(player.name || "").trim() || null,
            active: true,
            sort_order: index,
      ...
      const participation = state.game.participation || {};
      const rosterByLocalId = new Map((state.setup?.roster || []).map(player => [String(player.id),player]));
      const participationRows = Object.entries(participation)
        .map(([localPlayerId,entry]) => {
          const remotePlayerId = playerMap.get(localPlayerId);
          if (!remotePlayerId || !["played","dnp"].includes(entry?.status)) return null;
          const player = rosterByLocalId.get(String(localPlayerId)) || {};
          return {
            game_id:game.id,
            player_id:remotePlayerId,
            status:entry.status,
            game_cap:String(player.gameCap || player.cap || "").trim() || null,
      ...
      const desiredParticipationIds = new Set(participationRows.map(row => row.player_id));
      const { data:existingParticipation, error:participationReadError } = await this.client
        .from("live_game_participation").select("player_id").eq("game_id",game.id);
      if (participationReadError) throw participationReadError;
      const staleParticipationIds = (existingParticipation || []).map(row=>row.player_id).filter(id=>!desiredParticipationIds.has(id));
      if (staleParticipationIds.length) {
        const { error } = await this.client.from("live_game_participation").delete().eq("game_id",game.id).in("player_id",staleParticipationIds);
        if (error) throw error;
      }
      if (participationRows.length) {
        const { error } = await this.client.from("live_game_participation").upsert(participationRows,{onConflict:"game_id,player_id"});
        if (error) throw error;
      }
      ...
      const eventRows = (state.game.events || []).map((event, index) => ({
        ...
        metrics: {
          ...
          previousTimeRemaining: event.previousTimeRemaining || null,
          groupmeImageUrl: event.groupMeImageUrl || null
        },
