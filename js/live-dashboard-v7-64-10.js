 ...
 * Adds reusable league/event series plus non-game planning slots so a team can stage
 * known game counts/dates before opponents or times are published.
 *
 * Preserves WPI 7.62.0 — Unified Organization Discovery & Scale.
...
(() => {
  ...
  let clubView = false;
  let gameDayQueue = [];
  let plannedGameSlots = [];
  let leagueEventOptions = [];
  ...
  function renderGameSeriesArchive() {
    ...
    container.innerHTML = Array.from(grouped.entries()).map(([season,seriesRows]) => {
      const cards = seriesRows.map(series => {
        const label = series.seriesType === "tournament" ? "Tournament" : series.seriesType === "season_schedule" ? "Regular season" : series.seriesType === "league_event" ? "League / event" : "Scrimmage weekend";
  ...
  function plannedDateLabel(value) {
  ...
  function renderGameKindFields() {
    ...
    $("gameLeagueFields").hidden = !league;
    const other = tournament && $("gameTournamentSelect").value === "__other__";
    $("gameTournamentOtherLabel").hidden = !other;
    const leagueNew = league && $("gameLeagueEventSelect").value === "__new__";
    $("gameLeagueEventNewLabel").hidden = !leagueNew;
  ...
  function populateLeagueEventSelect(selectedName = "") {
    const select = $("gameLeagueEventSelect");
    if (!select) return;
    const names = [...new Set((leagueEventOptions || []).map(row => String(row.name || "").trim()).filter(Boolean))].sort((a,b) => a.localeCompare(b));
    if (selectedName && !names.some(name => name.toLowerCase() === String(selectedName).toLowerCase())) names.unshift(selectedName);
    select.innerHTML = names.map(name => `<option value="${escapeHtml(name)}">${escapeHtml(name)}</option>`).join("") + '<option value="__new__">Create new league / event…</option>';
    if (selectedName) select.value = names.find(name => name.toLowerCase() === String(selectedName).toLowerCase()) || "__new__";
    else if (names.length) select.value = names[0];
    else select.value = "__new__";
    $("gameLeagueEventNew").value = select.value === "__new__" ? selectedName : "";
  }

  function selectedLeagueEventName() {
    const select = $("gameLeagueEventSelect");
    if (!select) return "";
    return select.value === "__new__" ? $("gameLeagueEventNew").value.trim() : select.value.trim();
  }
  ...
  async function openGameDayDialog(game = null, plannedSlot = null) {
    await loadGameCatalog();
    ...
    editingPlannedSlotId = plannedSlot?.id || game?.plannedGameSlotId || null;
    $("gameDayDialogKicker").textContent = plannedSlot ? "Planned game" : (game ? "Edit game" : "Game-Day Hub");
    $("gameDayDialogTitle").textContent = plannedSlot ? "Add schedule details" : (game ? "Update game details" : "Add game");
    $("gameDayDialogHelp").textContent = plannedSlot
      ? `${plannedSlot.seriesName} already has this game slot saved for ${plannedDateLabel(plannedSlot.plannedDate)}. Add the opponent now; the exact start time can stay blank until it is published.`
      : game
        ? "You can change matchup details until the game starts. Live and completed records are protected from Game-Day Hub rewrites."
        : isHighSchoolWorkspace() ? "Choose Regular season for school schedule games. Tournament, League / event, and Friendly remain available when needed." : "Choose Tournament, League / event, or Friendly. League events such as BAWPL can span multiple weekends without becoming fake tournaments.";
    const kind = plannedSlot ? "league" : (game?.gameKind === "scrimmage" ? "friendly" : (game?.gameKind || (isHighSchoolWorkspace() ? "regular_season" : "tournament")));
    ...
    const existingLeagueName = plannedSlot?.seriesName || (game?.seriesType === "league_event" || game?.gameKind === "league" ? (game?.seriesName || game?.leagueEventName || "") : "");
    populateLeagueEventSelect(existingLeagueName);
    if (plannedSlot) $("gameLeagueEventSelect").disabled = true; else $("gameLeagueEventSelect").disabled = false;
    const plannedHint = $("gamePlannedDateHint");
    if (plannedHint) {
      const dateValue = plannedSlot?.plannedDate || game?.plannedDate || "";
      plannedHint.hidden = !dateValue;
      plannedHint.textContent = dateValue ? `Planned date: ${plannedDateLabel(dateValue)}. Leave time blank if the schedule has not published it yet.` : "";
    }
  ...
  function gameDayFormPayload() {
    ...
    const scrimmageWeekendName = kind === "friendly"
      ...
      : null;
    const leagueEventName = kind === "league" ? selectedLeagueEventName() : null;
    ...
    if (kind === "league" && !leagueEventName) throw new Error("Choose an existing league/event or create a new one.");
    ...
    return {
      ...
      tournamentName,
      seriesName:kind === "tournament" ? tournamentName : (kind === "league" ? leagueEventName : scrimmageWeekendName),
  ...
  async function saveGameDay({startAfter=false} = {}) {
    ...
    try {
      const {data:result,error} = await backend.client.rpc("live_save_game_day_v4", {
        target_team_id:workspace.teamId,
        ...
        target_planned_slot_id:editingPlannedSlotId || null
      });
      if (error) throw error;
  ...
  function gameDaySourceLabel(game) {
    ...
    if (game.gameKind === "league") return game.seriesName ? `League / event · ${game.seriesName}` : "League / event";
  ...
  async function planEventGameSlots() {
    if (!backend || !workspace || !["owner","admin"].includes(workspace.role)) return;
    const message = $("planGameSlotsMessage");
    const existing = $("planGameEventSelect").value;
    const eventName = existing === "__new__" ? $("planGameEventName").value.trim() : existing.trim();
    const plannedDate = $("planGameDate").value;
    const gameCount = Number($("planGameCount").value || 1);
    if (!eventName) { message.textContent = "Enter or choose the league/event name."; return; }
    if (!plannedDate) { message.textContent = "Choose the date."; return; }
    message.textContent = "Saving planned game slots…";
    try {
      const {error} = await backend.client.rpc("live_plan_game_slots_v1", {target_team_id:workspace.teamId,requested_series_name:eventName,requested_planned_date:plannedDate,requested_game_count:gameCount});
      if (error) throw error;
      $("planGameSlotsDialog").close();
      await loadGameDayQueue();
      $("gameDayQueueMessage").textContent = `${gameCount} planned ${eventName} game${gameCount === 1 ? "" : "s"} saved for ${plannedDateLabel(plannedDate)}. Opponents and times remain intentionally blank until the schedule is published.`;
    } catch (error) { message.textContent = error.message || "The planned game slots could not be saved."; }
  }

  function openPlanGameSlotsDialog() {
    const select = $("planGameEventSelect");
    const names = [...new Set((leagueEventOptions || []).map(row => String(row.name || "").trim()).filter(Boolean))].sort((a,b) => a.localeCompare(b));
    select.innerHTML = names.map(name => `<option value="${escapeHtml(name)}">${escapeHtml(name)}</option>`).join("") + '<option value="__new__">Create new league / event…</option>';
    const bawpl = names.find(name => name.toLowerCase() === "bawpl");
    select.value = bawpl || (names[0] || "__new__");
    $("planGameEventName").value = select.value === "__new__" ? "BAWPL" : "";
    $("planGameEventNameLabel").hidden = select.value !== "__new__";
    $("planGameDate").value = "";
    $("planGameCount").value = "2";
    $("planGameSlotsMessage").textContent = "";
    $("planGameSlotsDialog").showModal();
  }

  async function cancelGameSlot(slotId) {
    const slot = plannedGameSlots.find(row => String(row.id) === String(slotId));
    if (!slot || !["owner","admin"].includes(workspace?.role)) return;
    if (!confirm(`Remove ${slot.seriesName} Game ${slot.slotNumber} planned for ${plannedDateLabel(slot.plannedDate)}? No scored game will be deleted.`)) return;
    const {error} = await backend.client.rpc("live_cancel_game_slot_v1", {target_slot_id:slotId});
    if (error) throw error;
    await loadGameDayQueue();
  }
  ...
  function renderGameDayHub() {
    ...
    const slots = plannedGameSlots.filter(slot => slot.status === "planned");
    const hub = $("dashboardGameDay");
    const totalOnDeck = deck.length + slots.length;
    if (hub) hub.dataset.hasGames = totalOnDeck ? "true" : "false";
    $("gameDayQueueCount").textContent = `${totalOnDeck} game${totalOnDeck === 1 ? "" : "s"} on deck`;
    ...
    $("gameDayQueueSummary").textContent = totalOnDeck
      ? `${slots.length} schedule detail${slots.length === 1 ? "" : "s"} pending · ${assignedCount} assigned · ${unassignedCount} real game${unassignedCount === 1 ? "" : "s"} need scorer.`
      : isHighSchoolWorkspace() ? "No upcoming games yet. Sync the school schedule or add a regular-season game." : "No upcoming games yet. Add a tournament/league game, or plan game slots before a schedule is published.";
    ...
    const slotCards = slots.map(slot => `<article class="live-game-day-card live-game-day-slot" data-game-slot-id="${escapeHtml(slot.id)}">
      <div class="live-game-day-card-top"><strong class="live-game-day-tournament">${escapeHtml(slot.seriesName || "League / event")}</strong><span class="live-game-day-status" data-state="upcoming">Details pending</span></div>
      <div class="live-game-day-matchup"><div class="live-game-day-team"><img src="${escapeHtml(safeGameLogo(workspace?.clubLogoUrl))}" alt="${escapeHtml(workspace?.teamDisplayLabel || workspace?.teamName || "Team")} logo" onerror="this.onerror=null;this.src='assets/branding/wpi-logo-mark.png'"><span>${escapeHtml(workspace?.teamDisplayLabel || workspace?.teamName || "Team")}</span></div><strong>vs</strong><div class="live-game-day-team live-game-day-team--opponent"><img src="assets/branding/wpi-logo-mark.png" alt="Opponent TBD"><span>Opponent TBD</span></div></div>
      <div class="live-game-day-meta"><span>${escapeHtml(plannedDateLabel(slot.plannedDate))}</span><span>Time TBD</span><span>Game ${escapeHtml(slot.slotNumber)}</span></div>
      <div class="live-game-day-source" data-state="not_applicable">Planned slot · not a scored/public game</div>
      <p class="live-game-day-assignment-note">WPI is holding the date and event only. No opponent, time, venue, score, or schedule data has been invented.</p>
      <div class="live-game-day-actions">${["owner","admin"].includes(workspace.role) ? `<button type="button" class="live-game-day-start" data-game-slot-fill="${escapeHtml(slot.id)}">Add schedule details</button><button type="button" class="live-danger-button" data-game-slot-cancel="${escapeHtml(slot.id)}">Remove slot</button>` : '<span>Waiting for schedule details</span>'}</div>
    </article>`).join("");
    const gameCards = deck.map(game => {
      ...
      const seriesTitle = game.gameKind === "regular_season" ? "Regular Season" : (game.seriesName || (game.gameKind === "tournament" ? (game.tournamentName || "Tournament") : game.gameKind === "league" ? "League / event" : "Scrimmage weekend"));
      ...
      return `<article class="live-game-day-card" data-game-day-id="${escapeHtml(game.id)}">
        ...
        <div class="live-game-day-meta"><span>${escapeHtml(game.scheduledAt ? gameDayTimeLabel(game.scheduledAt) : (game.plannedDate ? `${plannedDateLabel(game.plannedDate)} · time TBD` : gameDayTimeLabel(null)))}</span>${game.venue ? `<span>${escapeHtml(game.venue)}</span>` : ""}<span>${escapeHtml(workspace.ageGroup || game.ageGroup || "")}</span></div>
    ...
    container.innerHTML = slotCards + gameCards;
  }

  async function loadGameDayQueue() {
    if (!backend || !workspace?.teamId) { gameDayQueue = []; return; }
    const {data,error} = await backend.client.rpc("live_game_day_queue_v6", {target_team_id:workspace.teamId});
    if (error) throw error;
    gameDayQueue = Array.isArray(data?.games) ? data.games : [];
    plannedGameSlots = Array.isArray(data?.plannedSlots) ? data.plannedSlots : [];
    leagueEventOptions = Array.isArray(data?.leagueEvents) ? data.leagueEvents : [];
    renderGameDayHub();
  }
  ...
  function renderDefaultLineup() {
    ...
    $("defaultLineupSummary").textContent = `Goalie ${rosterPlayerLabel(model.goalie)} · Field: ${fields.map(rosterPlayerLabel).join(", ")}`;
  }
  ...
  function renderCurrentRoster() {
    ...
    $("currentRosterList").innerHTML = count ? currentRoster.map(player => `
      <div class="live-current-roster-player">
        <strong>${escapeHtml(normalizedCap(player.cap) ? `#${normalizedCap(player.cap)}` : "Name only")}</strong>
        <span>${escapeHtml(cleanedRosterName(player.name) || (normalizedCap(player.cap) ? "Cap-only player" : "Unnamed player"))}</span>
  ...
  function openRosterDialog({source = "manual", rows = null} = {}) {
    ...
    $("rosterDialogHelp").textContent = source === "photo"
      ...
      : "Enter a cap number, player name, or both. Player identity stays stable even when game-day cap assignments change.";
    renderRosterDraft();
  ...
  function validateRosterDraft() {
    ...
    if (incomplete) return {ok:false, message:"Each roster row needs a cap number, player name, or both."};
    const caps = new Set();
    for (const row of complete) {
      ...
      if (cap && caps.has(cap)) return {ok:false, message:`Cap #${cap} appears more than once. Correct the duplicate before saving.`};
      if (cap) caps.add(cap);
    }
  ...
  function openDefaultLineupDialog() {
    ...
    $("defaultLineupGoalie").innerHTML = `<option value="">Select goalie</option>${currentRoster.map(player => `<option value="${escapeHtml(player.remoteId)}">${escapeHtml(rosterPlayerLabel(player))}</option>`).join("")}`;
  ...
  $("planGameSlotsButton")?.addEventListener("click", openPlanGameSlotsDialog);
  $("planGameEventSelect")?.addEventListener("change", event => { $("planGameEventNameLabel").hidden = event.target.value !== "__new__"; });
  $("confirmPlanGameSlotsButton")?.addEventListener("click", planEventGameSlots);
  ...
  $("gameLeagueEventSelect")?.addEventListener("change", renderGameKindFields);
  $("gameLeagueEventNew")?.addEventListener("input", renderGameDaySafety);
  ...
  $("gameDayQueue").addEventListener("click", event => {
    ...
    const fillSlot = event.target.closest("[data-game-slot-fill]");
    if (fillSlot) {
      const slot = plannedGameSlots.find(row => String(row.id) === String(fillSlot.dataset.gameSlotFill));
      return void openGameDayDialog(null,slot).catch(error => { $("dashboardConnectionDetail").textContent = error.message; });
    }
    const cancelSlot = event.target.closest("[data-game-slot-cancel]");
    if (cancelSlot) return void cancelGameSlot(cancelSlot.dataset.gameSlotCancel).catch(error => { $("dashboardConnectionDetail").textContent = error.message; });
