/* WPI 7.56.0 — regulation, 1–3 minute overtime, offensive shot outcomes, and shootout flow.
...
(() => {
  ...
  const RELEASE = "7.56.2";
  const STORAGE_KEY = "wpi-live-sandbox-v7-56-2";
  const LEGACY_STORAGE_KEYS = ["wpi-live-sandbox-v7-56-1","wpi-live-sandbox-v7-56-0","wpi-live-sandbox-v7-55-9", "wpi-live-sandbox-v7-55-8", "wpi-live-sandbox-v7-55-6", "wpi-live-sandbox-v7-55-5", "wpi-live-sandbox-v7-55-4", "wpi-live-sandbox-v7-55-3", "wpi-live-sandbox-v7-55-2"];
  const AUTH_KEY = "wpi-live-auth-v7-56-2";
  const LEGACY_AUTH_KEYS = ["wpi-live-auth-v7-56-1","wpi-live-auth-v7-56-0","wpi-live-auth-v7-55-9", "wpi-live-auth-v7-55-8", "wpi-live-auth-v7-55-6", "wpi-live-auth-v7-55-5", "wpi-live-auth-v7-55-4", "wpi-live-auth-v7-55-3"];
  ...
  async function deliverPendingMessages(result, forceEventIds = new Set()) {
    if (!backend || !groupMeDestination?.enabled || state.game.messagesPaused) return;
    const eventMap = result?.remoteEventMap || {};
    for (const message of [...(state.game.messages || [])].reverse()) {
      const remoteEventId = eventMap[message.eventId];
  ...
  async function pushRemoteState() {
    ...
    try {
      ...
      if (!unsubscribeRemote) unsubscribeRemote = backend.subscribeToGame(result.remoteGameId, remoteState => {
      ...
      $("connectionDetail").textContent = `Local copy preserved · ${error.message}`;
    } finally {
  ...
  function updateSubmitState() {
    const ready = canSubmitEvent();
    $("recordEventButton").disabled = !ready;
    $("recordEventButton").textContent = "Submit play";
  }
  ...
  async function authorize() {
    ...
    try {
      ...
      if (workspace.role === "viewer" && !remoteGameId) {
      ...
      if (remoteGameId) {
        ...
        if (loaded.state && loaded.state.setup && loaded.state.game) {
          ...
          unsubscribeRemote = backend.subscribeToGame(loaded.remoteGameId, remoteState => {
            if (!remoteState || remoteSyncInFlight) return;
            state = remoteState;
            state.release = RELEASE;
            state.mode = "connected";
      ...
      $("connectionLabel").textContent = "Connected private workspace";
      $("connectionDetail").textContent = `${session.user.email} · ${workspace.role} · ${workspace.teamName} · ${groupMeDestination?.enabled ? "GroupMe connected" : "GroupMe setup needed"}`;
      setSyncStatus(remoteGameId ? "Synced" : "Ready to save", remoteGameId ? "saved" : "local");
      return true;
    } catch (error) {
  ...
  function applyRoleAccess() {
    ...
    document.body.classList.add("is-live-viewer");
    ...
    $("connectionDetail").textContent = `${workspace.teamName} · Viewer · read-only private game history`;
  ...
  async function init() {
    ...
    renderAll();
    applyRoleAccess();
    if (backend && workspace && workspace.role !== "viewer" && !state.game.remoteId) scheduleRemoteSync(150);
  }
