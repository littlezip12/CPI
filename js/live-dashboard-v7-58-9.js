 ...
 * Scales Owner/Admin club operations with team filters, club-wide People & Access,
 * a deliberate unlisted-opponent review queue, and faster team creation while
 * preserving validated scorer, GroupMe, Following, archive and tournament semantics.
 *
...
(() => {
  ...
  function renderFollowing() {
    ...
    $("followingSummary").textContent = followed.length
      ...
      : "Find another Lamorinda team to follow";
    ...
    const visible = teams.filter(team => {
      ...
      const label = `${team.teamDisplayLabel || ""} ${team.teamName || ""} ${team.ageGroup || ""} ${team.gender || ""}`.toLowerCase();
      return (!query || label.includes(query)) && (age === "all" || String(team.ageGroup || "") === age) && (gender === "all" || normalizedGender(team.gender) === gender);
    });

    if (!teams.length) {
      $("followingTeamList").innerHTML = '<p class="live-empty-state">No additional Lamorinda teams are available yet.</p>';
      return;
    }
    $("followingTeamList").innerHTML = visible.length ? visible.map(team => {
      const label = team.teamDisplayLabel || team.teamName || "Team";
      const meta = [team.ageGroup, normalizedGender(team.gender)==="boys" ? "Boys" : normalizedGender(team.gender)==="girls" ? "Girls" : normalizedGender(team.gender)==="coed" ? "Coed" : null, team.squadLabel].filter(Boolean).join(" · ");
  ...
  function renderClubTeamGrid() {
    ...
    $("clubTeamGrid").innerHTML = teams.length ? teams.map(team => {
      ...
      const readyCopy = issues.length ? `Needs attention: ${issues.join(" · ")}` : "Roster, scorer access and GroupMe are ready.";
      return `<article class="live-club-team-card" data-state="${issues.length ? "attention" : "ready"}">
        <div class="live-club-team-card-heading"><div><small>${escapeHtml(team.competitiveSeason || "Season")} · ${escapeHtml([team.ageGroup,team.gender,team.squadLabel].filter(Boolean).join(" "))}</small><h3>${escapeHtml(team.teamDisplayLabel || team.teamName)}</h3><span>${escapeHtml(role)}</span></div><em>${issues.length ? `${issues.length} item${issues.length === 1 ? "" : "s"}` : "Ready ✓"}</em></div>
        ...
        <div class="live-club-team-actions">${membership ? `<button type="button" data-club-team-jump="${escapeHtml(team.teamId)}">Open team</button><button type="button" data-club-access-jump="${escapeHtml(team.teamId)}">Access</button>` : '<button type="button" disabled>Team membership required</button>'}</div>
      </article>`;
  ...
  function renderClubIdentityReview() {
    ...
    target.innerHTML = rows.length ? rows.map(row => {
      ...
      return `<article class="live-club-identity-row"><div><strong>${escapeHtml(row.opponentName || "Unlisted opponent")}</strong><span>${Number(row.gameCount || 0)} game${Number(row.gameCount || 0) === 1 ? "" : "s"} · ${escapeHtml(teamLabels || "Club game")} · last seen ${escapeHtml(last)}</span><small>Raw name preserved · never auto-merged into WPI identity</small></div>${href ? `<a href="${escapeHtml(href)}">Review latest game</a>` : ""}</article>`;
  ...
  function renderReadinessGuidance(model) {
    ...
    if (allReady) {
      ...
      $("readinessNextCopy").textContent = "Profile, roster, scoring access and score updates are ready for this team.";
  ...
  async function init() {
    ...
    try {
      ...
      if (clubView) {
        ...
        renderClubOverview();
        await Promise.all([loadClubPilotValidation(),loadClubOperations()]);
        return;
      }
      ...
      await Promise.all([loadGames(), loadGroupMe(), loadRoster(), loadFollowing()]);
      updateReadiness();
  ...
  $("refreshClubOperationsButton")?.addEventListener("click", () => loadClubOperations());
