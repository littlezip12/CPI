/* WPI 7.64.1 — Public score-only game viewer + tournament navigation + youth-safe direct sponsorship. */
(async()=>{
  "use strict";
  const $=id=>document.getElementById(id); const gameId=new URLSearchParams(location.search).get("game")||""; let backend=null,directory={organizations:[],teams:[]},refreshing=false,lastAdRotation="",adSuppressed=false,adAccessChecked=false;
  ...
  async function updateAdAccess(g){
    if(adAccessChecked||!backend||!g?.teamId)return;adAccessChecked=true;
    try{const {data:sessionData}=await backend.client.auth.getSession();if(!sessionData?.session)return;const {data,error}=await backend.client.rpc("live_analytics_access_level_v1",{target_team_id:g.teamId});if(!error&&["team_insights","organization_insights"].includes(String(data||"")))adSuppressed=true;}catch(e){console.warn(e);}
  }
  async function renderGameAd(g){
    const slot=$("publicGameAd");if(!slot||!window.WPILiveAds)return;await updateAdAccess(g);if(adSuppressed){slot.hidden=true;slot.replaceChildren();return;}
    const rotation=g.status==="live"?period(g):g.status==="final"?"final":"upcoming";if(rotation===lastAdRotation)return;lastAdRotation=rotation;
    await window.WPILiveAds.renderBanner(backend.client,slot,{gameId:g.id,rotationKey:rotation});
  }
  function render(g){
    ...
    const tournamentLink=$("publicScoreTournament");if(tournamentLink){const show=g.gameKind==="tournament"&&g.tournamentName;tournamentLink.hidden=!show;if(show){const u=new URL("live-tournament.html",location.href);u.search="";u.hash="";if(g.tournamentPublicId)u.searchParams.set("id",g.tournamentPublicId);else{u.searchParams.set("name",g.tournamentName);if(g.competitiveSeason)u.searchParams.set("season",g.competitiveSeason);}tournamentLink.href=u.pathname.split("/").pop()+u.search;tournamentLink.textContent="Tournament center";}}
    renderGameAd(g);
  }
