/* WPI 7.62.3 — Public Live Center. Score summaries only; no roster/player/scorer data. */
(async()=>{
  "use strict";
  ...
  function card(g){
    ...
    return `<article class="public-live-game" style="--game-primary:${esc(primary)};--game-secondary:${esc(secondary)}">
      <a class="public-live-game-link" href="live-score.html?game=${encodeURIComponent(g.id)}" aria-label="Open ${esc(g.teamDisplayLabel||g.teamName)} versus ${esc(g.opponentName||'opponent')}">
        <header><div class="public-live-game-team"><img src="${esc(logo)}" alt=""><div><small>${esc(meta(g))}</small><strong>${esc(g.teamDisplayLabel||g.teamName||"WPI team")}</strong></div></div><span class="public-live-status ${status}">${statusText}</span></header>
        <div class="public-live-matchup"><span>${esc(g.teamDisplayLabel||g.teamName||"Team")}</span><b>${esc(score)}</b><span>${esc(g.opponentName||"Opponent")}</span></div>
