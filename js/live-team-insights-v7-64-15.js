(() => {
  ...
  const livePlayerStatsExperienceRelease = "7.64.15";
  ...
  function scopedPlayer(id) {
    ...
    return found ? {...found,scopeMissing:false} : null;
  }
  function participationText(p) {
    const scopeType = playerScopeData?.scope?.type || playerScopeMode;
    if (scopeType === "game" && Number(p.games||0) === 0) return "DNP";
  ...
  function playerPickerLabel(p) {
    return `<span class="insights-player-pick-text"><strong>${esc(p?.name || "Player")}</strong><small>${esc(p?.rosterStatus || "Rostered for this scope")}</small></span>`;
  }
  function renderPlayerPicker() {
    const query = String($("playerSearchInput").value || "").trim().toLowerCase();
    const all = (Array.isArray(playerScopeData?.players) ? playerScopeData.players : []).filter(p => !query || `${p.name||""}`.toLowerCase().includes(query));
    const visibleSelectedCount = selectedPlayerIds.filter(id => all.some(p => String(p.playerId) === String(id))).length;
    $("playerSelectionCount").textContent = `${visibleSelectedCount} selected`;
  ...
  function syncLegacyPlayerControls() {
    const all=Array.isArray(playerScopeData?.players) ? playerScopeData.players : [];
  ...
  async function fetchPlayerAnalytics(scopeValue) {
    ...
    const {data,error}=await backend.client.rpc("live_team_player_insights_v2",{target_team_id:state.team.id,requested_season:state.selectedSeason||null,requested_scope:scope.type,requested_scope_id:scope.id||null});
    if(error) throw error;
    return data || {players:[]};
  }
