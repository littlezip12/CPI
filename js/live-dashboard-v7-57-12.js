/* WPI 7.57.12 — Supporter & Scorer Experience. Database role `viewer` remains unchanged for compatibility. */
(() => {
  "use strict";
  ...
  function renderRoleHome() {
    ...
    if (supporter) {
      ...
      $("roleHomeHelp").textContent = "Live games appear first, followed by upcoming games and recent finals. Open a live or completed game for the score, clock and play-by-play.";
    } else {
      $("roleHomeKicker").textContent = "Game-day scoring";
      $("roleHomeTitle").textContent = "Your games";
      $("roleHomeHelp").textContent = "Games assigned to you appear first. You can also claim an unassigned game when you are covering it.";
    }

    const statusRank = game => game.status === "live" ? 0 : (["setup","scheduled"].includes(game.status) ? 1 : 2);
    const scheduledTime = game => {
      const value = Date.parse(game.scheduledAt || game.startedAt || game.finalAt || 0);
      return Number.isFinite(value) ? value : 0;
    };
    let rows = gameDayQueue.filter(game => game.status !== "cancelled").filter(game => {
      if (supporter) return ["live","final","scheduled","setup"].includes(game.status);
      return game.status === "live" || game.isAssignedToMe || (!game.assignedScorerUserId && ["setup","scheduled"].includes(game.status));
    });
    rows.sort((a,b) => {
      if (scorer) {
        const scorerRank = game => game.status === "live" ? 0 : (game.isAssignedToMe ? 1 : 2);
        const rankDiff = scorerRank(a) - scorerRank(b);
        if (rankDiff) return rankDiff;
      }
      const statusDiff = statusRank(a) - statusRank(b);
      if (statusDiff) return statusDiff;
      if (a.status === "final" && b.status === "final") return scheduledTime(b) - scheduledTime(a);
    ...
    list.innerHTML = rows.map(game => {
      ...
      if (supporter && game.status === "live") action = `<a class="live-primary-link" href="${escapeHtml(teamScopedUrl("live-sandbox.html", {game:game.id}))}">Follow live</a>`;
      else if (supporter && game.status === "final") action = `<a class="live-secondary-link" href="${escapeHtml(teamScopedUrl("live-sandbox.html", {game:game.id}))}">View final</a>`;
      ...
      return `<article class="live-role-game-card" data-role-game-id="${escapeHtml(game.id)}">
        <div class="live-role-game-top"><span>${escapeHtml(game.gameKind === "tournament" ? (game.tournamentName || "Tournament") : "Friendly")}</span><div>${coverage}<span class="live-game-day-status" data-state="${escapeHtml(status.state)}">${escapeHtml(status.label)}</span></div></div>
