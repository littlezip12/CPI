(() => {
  ...
  async function applyEffectiveCapContext(gameId,data) {
    try {
      const {data:context,error} = await backend.client.rpc("live_game_player_cap_context_v1",{target_game_id:gameId});
      if (error || !context || !Array.isArray(context.players)) return data;
      const caps = new Map(context.players.map(row => [String(row.playerId || ""),row.effectiveCap]));
      const apply = row => {
        if (row?.playerId && caps.has(String(row.playerId))) row.cap = caps.get(String(row.playerId));
        return row;
      };
      (data.playerStats || []).forEach(apply);
      (data.events || []).forEach(row => {
        if (row?.playerId && caps.has(String(row.playerId))) row.playerCap = caps.get(String(row.playerId));
        if (row?.secondaryPlayerId && caps.has(String(row.secondaryPlayerId))) row.secondaryPlayerCap = caps.get(String(row.secondaryPlayerId));
      });
      (data.lineups || []).forEach(row => {
        if (row?.goalieId && caps.has(String(row.goalieId))) row.goalieCap = caps.get(String(row.goalieId));
        (row.players || []).forEach(player => { if (player?.id && caps.has(String(player.id))) player.cap = caps.get(String(player.id)); });
      });
      return data;
    } catch (_) { return data; }
  }
  ...
  function render(data) {
    ...
    $("recapSeriesName").textContent = series.name || (game.gameKind === "tournament" ? "Tournament" : game.gameKind === "league" ? "League / Event" : game.gameKind === "regular_season" ? "Regular Season" : "Scrimmage Weekend");
    $("recapSeason").textContent = series.competitiveSeason || game.competitiveSeason || "Season";
    $("recapEventDetails").innerHTML = [
      metaItem("Type",series.seriesType === "tournament" ? "Tournament" : series.seriesType === "league_event" ? "League / Event" : series.seriesType === "season_schedule" ? "Regular Season" : "Scrimmage Weekend"),
    ...
    if (!hasDetailedAnalytics) {
      ...
      $("recapLineups").innerHTML = lineups.length ? lineups.map(row => {
        const goalieIdentity = row.goalieCap && row.goalieName ? `#${row.goalieCap} ${row.goalieName}` : row.goalieCap ? `#${row.goalieCap}` : row.goalieName || "not recorded";
        const goalie = `Goalie: ${goalieIdentity}`;
        const players = (row.players || []).map(p => p.cap && p.name ? `#${p.cap} ${p.name}` : p.cap ? `#${p.cap}` : p.name || "Unnamed player").join(" · ");
      ...
      $("recapPlayerStats").innerHTML = stats.length ? stats.map(row => {
        ...
        const identity = row.cap && row.name ? `#${row.cap} ${row.name}` : row.cap ? `#${row.cap}` : row.name || "Unnamed player";
        return `<div class="live-recap-stat-row"><strong>${esc(identity)}</strong>${chips.length ? `<div class="live-recap-stat-chips">${chips.map(([k,v]) => `<em>${esc(k)} ${esc(v)}</em>`).join("")}</div>` : '<span>No tracked counting stats.</span>'}</div>`;
  ...
  async function init() {
    try {
      ...
      const recapData = await applyEffectiveCapContext(gameId,data || {});
      render(recapData);
