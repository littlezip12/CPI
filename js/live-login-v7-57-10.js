/* WPI 7.57.10 — least-privilege registration copy on the validated 7.56.8 auth gateway. */
(() => {
  "use strict";
  ...
  function setMode(nextMode) {
    ...
    $("loginExplanation").textContent = signingUp
      ? "Use your own email and password. New team members join as Viewer; a Team Owner or Admin can grant Scorer access after you join."
      : "Sign in once, then choose any team workspace your account can access.";
  ...
  async function initConnectedMode() {
    ...
    try {
      ...
      if (!signupAllowed) {
        $("loginExplanation").textContent = "Sign in with an existing account. New accounts require a private team invitation and join with Viewer access first.";
      }
    } catch (error) {
