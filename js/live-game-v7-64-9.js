...
(() => {
  ...
  function availableRoster() {
    return state.setup.roster.filter(player => String(effectivePlayerCap(player) || "").trim() || String(player.name || "").trim());
  }
  ...
  function addRosterPlayer() {
    state.setup.roster = rosterFromForm();
    const player = {id:uid("player"), cap:"", gameCap:"", name:"", createdByUserId:backend?.user?.id || null};
