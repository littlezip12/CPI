/* WPI 7.62.3 — Public score-only game viewer. */
(async()=>{
  "use strict";
  ...
  function render(g){
    ...
    $("publicScoreLogo").src=g.teamLogoUrl||org?.logo||"assets/logos/cpi-logo-fallback.svg";$("publicScoreLogo").alt=`${g.organizationName||"WPI"} logo`;
    $("publicScoreOrganization").textContent=g.organizationName||"WPI Live";$("publicScoreTeam").textContent=g.teamDisplayLabel||g.teamName||"WPI team";$("publicScoreMeta").textContent=[g.ageGroup==="HS"?"High School":g.ageGroup,g.gender,g.squadLabel].filter(Boolean).join(" · ")||"WPI Live";
    $("publicScoreHome").textContent=g.teamDisplayLabel||g.teamName||"Team";$("publicScoreOpponent").textContent=g.opponentName||"Opponent";$("publicScoreHomeValue").textContent=g.teamScore??0;$("publicScoreOpponentValue").textContent=g.opponentScore??0;
    ...
    $("publicScoreUpdated").textContent=`Updated ${new Date().toLocaleTimeString([],{hour:"numeric",minute:"2-digit",second:"2-digit"})}`;$("publicScoreDetails").textContent=[dateLabel(g.scheduledAt),g.venue,g.tournamentName].filter(Boolean).join(" · ");
