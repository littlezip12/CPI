/* WPI 7.55.3 — live-scoring auth gateway and mobile workflow refinement.
 * Sandbox data stays quarantined from rankings and official tournament data.
 */
(() => {
  "use strict";

  const RELEASE = "7.55.3";
  const STORAGE_KEY = "wpi-live-sandbox-v7-55-3";
  ...
  const EVENT_TYPES = [
    ...
    {id:"save",label:"Save",category:"defense",requiresPlayer:true,allowsSecondary:false,priority:"all"},
    {id:"field_block",label:"Field block",category:"defense",requiresPlayer:true,allowsSecondary:false,priority:"all"},
    {id:"steal",label:"Steal",category:"possession",requiresPlayer:true,allowsSecondary:false,priority:"all"},
    {id:"turnover",label:"Turnover",category:"possession",requiresPlayer:true,allowsSecondary:false,priority:"all"},
    {id:"exclusion_drawn",label:"Exclusion drawn",category:"exclusions",requiresPlayer:true,allowsSecondary:false,priority:"major"},
    {id:"exclusion_committed",label:"Exclusion called on",category:"exclusions",requiresPlayer:true,allowsSecondary:false,priority:"major"},
    {id:"five_meter_drawn",label:"5m drawn",category:"penalties",requiresPlayer:true,allowsSecondary:false,priority:"major"},
    {id:"five_meter_committed",label:"5m called on",category:"penalties",requiresPlayer:true,allowsSecondary:false,priority:"major"}
  ];
  const EVENT_MAP = new Map(EVENT_TYPES.map(item => [item.id, item]));

  const defaultRoster = () => Array.from({length: 12}, (_, index) => ({
    id: `p${index + 1}`,
    cap: String(index + 1),
  ...
  function scheduledGames() {
    return Array.isArray(config.scheduledGames) ? config.scheduledGames : [];
  }

  function showGameSource(mode) {
    const scheduled = mode === "scheduled";
    $("scheduledGameTab").setAttribute("aria-selected", String(scheduled));
    $("scrimmageTab").setAttribute("aria-selected", String(!scheduled));
    $("scheduledGamePanel").hidden = !scheduled;
    $("scrimmagePanel").hidden = scheduled;
    if (!scheduled) {
      state.setup.sourceMode = "manual";
      state.setup.sourceGameId = null;
      state.setup.tournamentName = "";
      $("setupSourceBadge").textContent = "Manual scrimmage";
      $("gameSourceLabel").textContent = "Manual entry";
      saveState();
    }
  }
  ...
  function renderScheduledGames() {
    ...
    if (!games.length) {
      $("scheduledGameList").innerHTML = `<div class="live-schedule-empty"><strong>No tournament games are connected yet.</strong><p>When the event schedule is connected, your team’s games will appear here with opponent, date, time, venue, and quarter length preloaded.</p><button type="button" data-use-scrimmage>Use manual scrimmage instead</button></div>`;
      $("scheduledGameList").querySelector("[data-use-scrimmage]").addEventListener("click", () => showGameSource("manual"));
      return;
    }

    $("scheduledGameList").innerHTML = games.map(game => `
      <button type="button" class="live-scheduled-game" data-scheduled-game="${escapeHtml(game.id)}">
        <span>${escapeHtml(game.tournament || "Tournament game")}</span>
        <strong>${escapeHtml(game.team || state.setup.teamName)} vs. ${escapeHtml(game.opponent || "Opponent pending")}</strong>
        <small>${escapeHtml(game.scheduledLabel || game.scheduledAt || "Time pending")} · ${escapeHtml(game.venue || "Venue pending")}</small>
      </button>`).join("");

    $("scheduledGameList").querySelectorAll("[data-scheduled-game]").forEach(button => button.addEventListener("click", () => {
      const game = games.find(item => item.id === button.dataset.scheduledGame);
      if (!game) return;
      state.setup.sourceMode = "tournament_sheet";
      state.setup.sourceGameId = game.id;
      state.setup.tournamentName = game.tournament || "Tournament game";
      state.setup.teamName = game.team || state.setup.teamName;
      state.setup.opponentName = game.opponent || "Opponent pending";
      state.setup.gameDateTime = game.scheduledAt || "";
      state.setup.venue = game.venue || "";
      state.setup.quarterLength = clamp(game.quarterLength || state.setup.quarterLength, 1, 15);
      saveState();
      fillSetupForm();
      $("setupSourceBadge").textContent = "Tournament schedule";
      $("gameSourceLabel").textContent = game.tournament || "Scheduled game";
      $("scrimmageTab").setAttribute("aria-selected", "false");
      $("scheduledGameTab").setAttribute("aria-selected", "true");
      $("scheduledGamePanel").hidden = false;
      $("scrimmagePanel").hidden = true;
      $("teamName").scrollIntoView({behavior:"smooth", block:"center"});
    }));
  }
  ...
  function renderActions() {
    ...
    $("actionTabs").querySelectorAll("[data-action]").forEach(button => button.addEventListener("click", () => {
      ...
      window.requestAnimationFrame(() => {
        $("actionEntryPanel").scrollIntoView({behavior:"smooth", block:"center"});
        const type = EVENT_MAP.get(selectedAction);
        window.setTimeout(() => (type.requiresPlayer ? $("primaryPlayer") : $("eventNote")).focus(), 250);
      });
    }));
  }
  ...
  function renderPlayerOptions() {
    ...
    const secondaryValue = $("secondaryPlayer")?.value || "";
    ...
    if (state.setup.roster.some(player => player.id === primaryValue)) $("primaryPlayer").value = primaryValue;
    if (state.setup.roster.some(player => player.id === secondaryValue)) $("secondaryPlayer").value = secondaryValue;
  }
  ...
  function renderActionEntry() {
    ...
    $("primaryPlayerLabel").toggleAttribute("hidden", !type.requiresPlayer);
    const allowAssist = type.id === "goal";
    $("secondaryPlayerLabel").toggleAttribute("hidden", !allowAssist);
    if (!allowAssist) $("secondaryPlayer").value = "";
  ...
  function baseEvent(type, overrides = {}) {
    return {
      ...
      secondaryPlayerId: type.id === "goal" ? (overrides.secondaryPlayerId || null) : null,
      ...
      status: "active",
      createdAt: new Date().toISOString(),
      createdBy: authenticatedUser?.email || (config.mode === "connected" ? "authenticated scorer" : "local demo scorer")
    };
  }

  function draftEvent() {
    const type = EVENT_MAP.get(selectedAction) || EVENT_TYPES[0];
    const playerId = type.requiresPlayer ? $("primaryPlayer").value : null;
    const secondaryPlayerId = type.id === "goal" ? $("secondaryPlayer").value : null;
    const event = baseEvent(type, {playerId, secondaryPlayerId, note: $("eventNote").value.trim()});
    event.scoreAfter = {
      team: state.game.teamScore + (type.teamDelta || 0),
      opponent: state.game.opponentScore + (type.opponentDelta || 0)
    };
    return {event, type};
  }

  function renderCurrentPreview() {
    if (!$("currentMessagePreview")) return;
    const {event, type} = draftEvent();
    if (type.requiresPlayer && !event.playerId) {
      $("currentMessagePreview").textContent = "Choose a player to preview this update.";
      $("currentPreviewStatus").textContent = "Player needed";
      return;
    }
    if (event.playerId && event.secondaryPlayerId === event.playerId) {
      $("currentMessagePreview").textContent = "The scorer and assist must be different players.";
      $("currentPreviewStatus").textContent = "Fix selection";
      return;
    }
    $("currentMessagePreview").textContent = formatMessage(event).replace(/\*\*/g, "");
    if (!shouldCreateMessage(type)) {
      $("currentPreviewStatus").textContent = state.game.messagesPaused ? "Messages paused" : "Recorded only";
    } else {
      $("currentPreviewStatus").textContent = config.groupMeDelivery === "live" ? "Will send to GroupMe" : "Mock GroupMe preview";
    }
  }
  ...
  function syncClockFromInputs(persist = true) {
  ...
  function renderAll() {
    ...
    const setup = state.game.status === "setup";
    $("setupPanel").hidden = !setup;
    $("liveConsole").hidden = setup;
    $("summaryPanel").hidden = state.game.status !== "ended";
    if (state.game.status === "ended") renderSummary();
  }
  ...
  function resetSandbox() {
    ...
    const setup = {...state.setup, roster: state.setup.roster.map(player => ({...player})), sourceMode:"manual", sourceGameId:null, tournamentName:""};
    state = defaultState();
    state.setup = setup;
    localStorage.removeItem(STORAGE_KEY);
    saveState();
    fillSetupForm();
    showGameSource("manual");
    renderAll();
    $("setupPanel").scrollIntoView({behavior:"smooth", block:"start"});
  }

  function showAuthorizedAccess(user = null) {
    authenticatedUser = user;
    document.body.classList.remove("live-access-pending");
    $("liveSandboxApp").hidden = false;
    $("connectionLabel").textContent = user ? "Authenticated sandbox" : "Local demo session";
    $("connectionDetail").textContent = user ? `${user.email} · secure account active; connected data storage follows in the backend release.` : "Browser-local test data. GroupMe messages are previewed but not sent.";
    renderAll();
  }

  function redirectToLogin() {
    window.location.replace(`live-login.html?next=${encodeURIComponent("live-sandbox.html")}`);
  }
  ...
  async function initAccess() {
    ...
    try {
      ...
      showAuthorizedAccess(data.session.user);
      ...
      $("liveSandboxApp").hidden = false;
      document.body.classList.remove("live-access-pending");
      $("connectionLabel").textContent = "Secure connection failed";
  ...
  function bindEvents() {
    ...
    $("scheduledGameTab").addEventListener("click", () => showGameSource("scheduled"));
    $("scrimmageTab").addEventListener("click", () => showGameSource("manual"));
    $("addPlayerButton").addEventListener("click", () => {
      syncSetupFromForm();
      state.setup.roster.push({id:uid("player"), cap:"", name:""});
      saveState();
      renderRoster();
      const rows = $("rosterList").querySelectorAll(".live-roster-row");
    ...
    $("startQuarterButton").addEventListener("click", () => {
      syncClockFromInputs();
      openLineupDialog(Number($("currentQuarter").value));
    });
    $("recordEventButton").addEventListener("click", recordSelectedEvent);
    ["currentQuarter","clockMinutes","clockSeconds"].forEach(id => {
      $(id).addEventListener("change", () => syncClockFromInputs());
      $(id).addEventListener("input", () => syncClockFromInputs(false));
    });
    ["primaryPlayer","secondaryPlayer"].forEach(id => $(id).addEventListener("change", renderCurrentPreview));
    $("eventNote").addEventListener("input", renderCurrentPreview);
    $("messageFrequency").addEventListener("change", renderCurrentPreview);
  ...
  async function init() {
    ...
    renderActionEntry();
    bindEvents();
    const accessGranted = await initAccess();
    if (!accessGranted) return;
    if (state.setup.sourceMode === "tournament_sheet") showGameSource("scheduled");
    else showGameSource("manual");
    renderAll();
  }
