 ...
 * Keeps permanent membership/permissions team-specific while adding a separate
 * read-only Lamorinda Following relationship for Supporters and Scorers.
 */
(() => {
  "use strict";
  ...
  function renderFollowing() {
    ...
    $("followingSummary").textContent = followed.length
      ...
      : "Choose another Lamorinda team to follow";
    ...
    $("followingTeamList").innerHTML = teams.map(team => {
      const label = team.teamDisplayLabel || team.teamName || "Team";
      if (team.isMember) {
        return `<article class="live-follow-team-row" data-state="member">
          <div><strong>${escapeHtml(label)}</strong><small>Your permanent team access · ${escapeHtml(roleLabel(team.membershipRole || "viewer"))}</small></div>
          <span class="live-follow-team-state">Member</span>
        </article>`;
      }
      const following = team.isFollowing === true;
      return `<article class="live-follow-team-row" data-state="${following ? "following" : "available"}">
        <div><strong>${escapeHtml(label)}</strong><small>${escapeHtml(team.competitiveSeason || "Current season")} · Following never grants scoring or admin access</small></div>
        <button type="button" data-follow-team="${escapeHtml(team.teamId)}" data-follow-next="${following ? "false" : "true"}">${following ? "Unfollow" : "Follow"}</button>
      </article>`;
    }).join("");
  }
  ...
  function renderRoleHome() {
    ...
    const available = supporter ? dedupe([...memberGames,...followedGames]) : memberGames;
    const followedOrdered = followedGames.slice().sort((a,b) => {
    ...
    const renderCard = game => {
      ...
      else if (followingGame && game.status === "final") action = `<a class="live-secondary-link" href="${escapeHtml(teamScopedUrl("live-game.html", {game:game.id,team:game.teamId,follow:1}))}">View final</a>`;
      ...
      const contextLabel = followingGame ? `${game.teamDisplayLabel || game.teamName || "Followed team"} · ${seriesLabel}` : seriesLabel;
      return `<article class="live-role-game-card" data-role-game-id="${escapeHtml(game.id)}" data-following-game="${followingGame ? "true" : "false"}">
        <div class="live-role-game-top"><span>${escapeHtml(contextLabel)}</span><div>${coverage}<span class="live-game-day-status" data-state="${escapeHtml(status.state)}">${escapeHtml(status.label)}</span></div></div>
  ...
  $("followingTeamList")?.addEventListener("click", event => {
