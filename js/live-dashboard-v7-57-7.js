...
(() => {
  ...
  async function prepareGameDayStart(gameId) {
    ...
    const {data,error} = await backend.client.rpc("live_prepare_game_start_v1", {target_game_id:gameId});
  ...
  async function loadGameDayQueue() {
    ...
    const {data,error} = await backend.client.rpc("live_game_day_queue_v2", {target_team_id:workspace.teamId});
    if (error) throw error;
