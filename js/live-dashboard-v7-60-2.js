...
(() => {
  ...
  function launchChecklistItems(team) {
    ...
    if (!row) {
      return [
        ...
        {key:"groupme",label:"GroupMe",ready:Boolean(team?.groupMeReady)}
      ];
    }
    return [
      ...
      {key:"groupme",label:"GroupMe",ready:Boolean(row.groupMeReady)},
      {key:"lineup",label:"Starters",ready:Boolean(row.defaultLineupReady)}
    ];
  }

  function clubTeamIssues(team) {
    const labels = {profile:"Profile",roster:"Roster",access:"Scorer access",groupme:"GroupMe",lineup:"Default starters"};
  ...
  function renderReadinessGuidance(model) {
    ...
    if (allReady) {
      ...
      $("readinessNextCopy").textContent = "Profile, roster, scoring access, score updates and default starters are ready for this team.";
