/* WPI 7.55.5 — one-hand guided live-scoring sandbox.
...
(() => {
  ...
  const RELEASE = "7.55.5";
  const STORAGE_KEY = "wpi-live-sandbox-v7-55-5";
  const LEGACY_STORAGE_KEYS = ["wpi-live-sandbox-v7-55-4", "wpi-live-sandbox-v7-55-3", "wpi-live-sandbox-v7-55-2"];
  const AUTH_KEY = "wpi-live-auth-v7-55-5";
  const LEGACY_AUTH_KEYS = ["wpi-live-auth-v7-55-4", "wpi-live-auth-v7-55-3"];
  ...
  function updateSubmitState() {
    ...
    $("recordEventButton").textContent = !type ? "Choose an event" : ready ? `Record ${type.label.toLowerCase()}` : type.requiresPlayer && !$("primaryPlayer").value ? "Choose a player" : "Check the time";
  }
  ...
  function renderScoreboard() {
    ...
    const nextQuarter = Math.min(5, Number(state.game.quarter) + 1);
    $("startNextQuarterButton").textContent = state.game.quarter >= 5 ? "Edit OT starters" : `Start ${quarterText(nextQuarter)} · set starters`;
    $("startNextQuarterButton").dataset.quarter = String(state.game.quarter >= 5 ? 5 : nextQuarter);

    const lineup = lineupForQuarter(state.game.quarter);
    $("currentLineupStatus").textContent = lineup.length ? `${quarterText()} · ${lineup.length} starters saved` : `${quarterText()} · lineup not set`;
  ...
  function bindEvents() {
    ...
    $("startNextQuarterButton").addEventListener("click", () => {
      if (!syncClockFromInput(true)) return;
      openLineupDialog(Number($("startNextQuarterButton").dataset.quarter || Math.min(5, state.game.quarter + 1)));
    });
