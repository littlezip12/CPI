...
(() => {
  ...
  async function saveGameDay({startAfter=false} = {}) {
    ...
    try {
      const {data:result,error} = await backend.client.rpc("live_save_game_day_v1", {
        target_team_id:workspace.teamId,
      ...
      if (startAfter) {
        ...
        window.location.assign(teamScopedUrl("live-sandbox.html", {game:gameId}));
        return;
      }
      $("gameDayDialog").close();
  ...
  function renderRoleHome() {
    ...
    panel.hidden = !(viewer || scorer);
    if (!(viewer || scorer)) return;

    if (viewer) {
      $("roleHomeKicker").textContent = "Team games";
      $("roleHomeTitle").textContent = "Follow a game";
      $("roleHomeHelp").textContent = "Choose a live or completed game to see the score, game clock and play-by-play. Your account is read-only unless a Team Owner/Admin changes your access.";
    } else {
      $("roleHomeKicker").textContent = "Game-day scoring";
      $("roleHomeTitle").textContent = "Your scoring assignments";
      $("roleHomeHelp").textContent = "Assigned and unassigned games appear below. Start a game when you are covering it; scorer handoff remains available once scoring begins.";
    }

    const rows = gameDayQueue.filter(game => game.status !== "cancelled").filter(game => {
      if (viewer) return ["live","final","scheduled","setup"].includes(game.status);
      return game.status === "live" || game.isAssignedToMe || (!game.assignedScorerUserId && ["setup","scheduled"].includes(game.status));
    });
    if (!rows.length) {
      list.innerHTML = `<div class="live-role-game-empty"><strong>${viewer ? "No games available yet" : "No games assigned yet"}</strong><span>${viewer ? "When a game is scheduled or starts, it will appear here." : "An Owner/Admin can assign a game, or you can claim an unassigned game from Game Day."}</span></div>`;
      return;
    }
    list.innerHTML = rows.map(game => {
      ...
      const score = ["live","final"].includes(game.status) ? `<strong class="live-role-game-score">${escapeHtml(String(game.teamScore ?? 0))}–${escapeHtml(String(game.opponentScore ?? 0))}</strong>` : "";
      let action = '<span class="live-role-game-waiting">Not started</span>';
      if (viewer && game.status === "live") action = `<a class="live-primary-link" href="${escapeHtml(teamScopedUrl("live-sandbox.html", {game:game.id}))}">Watch live</a>`;
      else if (viewer && game.status === "final") action = `<a class="live-secondary-link" href="${escapeHtml(teamScopedUrl("live-sandbox.html", {game:game.id}))}">View game</a>`;
      ...
      return `<article class="live-role-game-card" data-role-game-id="${escapeHtml(game.id)}">
        <div class="live-role-game-top"><span>${escapeHtml(game.gameKind === "tournament" ? (game.tournamentName || "Tournament") : "Friendly")}</span><span class="live-game-day-status" data-state="${escapeHtml(status.state)}">${escapeHtml(status.label)}</span></div>
  ...
  function updateInvitePermissionVisibility() {
    // 7.57.10: invitations are always Viewer-first. Elevated permissions are granted only after acceptance.
  }
  ...
  async function createInvite() {
    ...
    $("inviteMessage").textContent = "Creating Viewer-first invite…";
    try {
      ...
      $("inviteEmail").value = "";
      $("inviteMessage").textContent = "Invite created. They will join as Viewer; promote access after they accept.";
  ...
  function applyRoleExperience() {
    ...
    if (workspace.role === "viewer") {
      $("dashboardSubtitle").textContent = "Choose a game to follow live. Viewer access is read-only.";
  ...
  async function init() {
    ...
    try {
      ...
      $("dashboardRoleBadge").textContent = roleLabel(workspace.role);
  ...
  $("roleHomeGames").addEventListener("click", event => {
    ...
    if (game?.status === "live") { window.location.assign(teamScopedUrl("live-sandbox.html", {game:gameId})); return; }
    start.disabled = true;
