/* WPI 7.60.3 — Self-Service Club Onboarding Auth Entry.
...
(() => {
  ...
  async function initConnectedMode() {
    ...
    try {
      ...
      if (session && backend.isAnonymousUser(session.user)) {
        ...
        window.location.replace(onboarding ? "live-club-onboarding.html" : (following ? "live-following.html" : `live-dashboard.html${invite ? `?invite=${encodeURIComponent(invite)}` : ""}`));
        return;
      }
  ...
  async function submitAuth(event) {
    ...
    try {
      const invite = params.get("invite");
      const dashboardRedirect = new URL(onboarding ? "live-club-onboarding.html" : (following ? "live-following.html" : "live-dashboard.html"), window.location.href);
      if (invite && !onboarding && !following) dashboardRedirect.searchParams.set("invite", invite);
      ...
      window.location.assign(onboarding ? "live-club-onboarding.html" : (following ? "live-following.html" : `live-dashboard.html${invite ? `?invite=${encodeURIComponent(invite)}` : ""}`));
    } catch (error) {
