/* WPI 7.64.1 — Public Live Center + Tournament Experience links. Score summaries only; no roster/player/scorer data. */
(async()=>{
  "use strict";
  ...
  function card(g){
    ...
    return `<article class="public-live-game" style="--game-primary:${esc(primary)};--game-secondary:${esc(secondary)}">
      ...
      <div class="public-live-game-actions"><div><a href="${esc(teamHref)}">Team hub</a>${tournamentHref?`<a href="${esc(tournamentHref)}">Tournament center</a>`:""}</div>${g.gameKind?`<span>${esc(g.gameKind.replaceAll("_"," "))}</span>`:""}</div>
    </article>`;
  }
  ...
  async function refresh(){
    ...
    try{const {data,error}=await backend.client.rpc("live_public_scoreboard_v2");if(error)throw error;games=Array.isArray(data?.games)?data.games:[];$("publicLastUpdated").textContent=new Date().toLocaleTimeString([],{hour:"numeric",minute:"2-digit"});render();}
