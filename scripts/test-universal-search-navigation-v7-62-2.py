...
req('release 7.62.2' in site,'site shell must identify the 7.62.2 release')
req('{ label: "Organizations", href: "organizations.html"' in site,'primary navigation must expose Organizations')
req('{ label: "WPI Live", href: "live-following.html"' in site,'primary navigation must expose WPI Live')
...
req(site_release.get('version')=='7.62.2','site release metadata must be 7.62.2')
req(site_release.get('navigationRelease')=='7.62.2','navigation release metadata must advance to 7.62.2')
...
for p in shell_pages:
    req('js/site-shell.js?v=7.62.2' in p.read_text(),f'{p.name} must cache-bust the 7.62.2 site shell')
for p in cmd_pages:
