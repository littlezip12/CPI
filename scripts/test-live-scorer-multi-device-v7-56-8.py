...
if site.get("version")!="7.56.8": errors.append("site version must be 7.56.8")
if site.get("liveScoringMultiDeviceRelease")!="7.56.8": errors.append("multi-device release marker missing")
if site.get("liveScoringRoleSecurityRelease")!="7.56.8": errors.append("role-security release marker missing")
...
for page,script in (
    ...
    if expected_backend not in text: errors.append(f"{page} missing 7.56.8 backend")
    if script not in text: errors.append(f"{page} missing {script}")
    if 'config/live-sandbox.js?v=7.56.8' not in text: errors.append(f"{page} missing 7.56.8 config cache bust")
...
if errors:
    print("WPI SCORER MULTI-DEVICE 7.56.8 TEST FAILED")
    for error in errors: print(f" - {error}")
    sys.exit(1)
print("WPI SCORER MULTI-DEVICE 7.56.8 TEST PASSED")
...
print(" - Private WPI Live assets are cache-busted to 7.56.8")
