...
for key, expected in {
    ...
    'publicExperienceRelease': '7.54.6',
...
for path in ROOT.rglob('*.html'):
    ...
    for match in re.finditer(r'\?v=(\d+(?:\.\d+){1,3}(?:-[A-Za-z0-9.-]+)?)', text):
        if match.group(1) not in {'7.53.4','7.53.5','7.53.6','7.53.7','7.54.0','7.54.1','7.54.2','7.54.3','7.54.4','7.54.5','7.54.6'}:
...
if errors:
    print('WPI 7.54.6 BRAND / TEAMS / SECTION LANDING TEST FAILED')
    for error in errors:
        print(f' - {error}')
    sys.exit(1)

print('WPI 7.54.6 BRAND / TEAMS / SECTION LANDING TEST PASSED')
