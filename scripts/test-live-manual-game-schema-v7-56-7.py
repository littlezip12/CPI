...
if site.get("version") != "7.56.7":
    errors.append("site release must be 7.56.7")
...
for token in (
    ...
    require(migration, token, "7.56.7 schema migration")
...
if legacy_inline in migration.lower():
    errors.append("7.56.7 migration must remove, not recreate, the legacy inline constraint")

if errors:
    print("WPI MANUAL GAME SCHEMA 7.56.7 TEST FAILED")
    for error in errors:
        print(f" - {error}")
    sys.exit(1)

print("WPI MANUAL GAME SCHEMA 7.56.7 TEST PASSED")
