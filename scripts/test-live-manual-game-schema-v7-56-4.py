...
if site.get("version") != "7.56.4":
    errors.append("site release must be 7.56.4")
if site.get("liveScoringManualGameSchemaRelease") != "7.56.4":
    errors.append("manual-game schema release must be 7.56.4")
...
for token in (
    ...
    require(migration, token, "7.56.4 schema migration")
...
if legacy_inline in migration.lower():
    errors.append("7.56.4 migration must remove, not recreate, the legacy inline constraint")

install = read("INSTALL_7.56.4.md")
for token in ("409 Conflict", "Do not rerun the full setup SQL", "live_games_official_source_idx", "live_games_team_client_id_idx"):
    require(install, token, "7.56.4 install guide")

if errors:
    print("WPI MANUAL GAME SCHEMA 7.56.4 TEST FAILED")
    for error in errors:
        print(f" - {error}")
    sys.exit(1)

print("WPI MANUAL GAME SCHEMA 7.56.4 TEST PASSED")
