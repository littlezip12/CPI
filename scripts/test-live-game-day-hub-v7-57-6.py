...
dashboard_js = (ROOT / "js/live-dashboard-v7-57-6.js").read_text()
css = (ROOT / "css/live-sandbox-v7-57-6.css").read_text()
...
check("site version", site.get("version") == "7.57.6")
check("release name", site.get("name") == "Game-Day Hub & Universal Game Model")
for key in [
    "liveScoringDashboardRelease","liveScoringTeamAdminRelease","liveScoringGameDayHubRelease",
    "liveScoringUniversalGameModelRelease","liveScoringManualTournamentFallbackRelease",
    "liveScoringReconciliationFoundationRelease"
]:
...
for token in [
    ...
    'Tournament game', 'Scrimmage / friendly', 'id="gameDayDialog"', 'Save to Game Day',
...
check("dashboard JS wired", 'js/live-dashboard-v7-57-6.js?v=7.57.6' in dashboard_html)
check("7.57.6 CSS wired", 'css/live-sandbox-v7-57-6.css?v=7.57.6' in dashboard_html)
...
for token in [
    ...
    'live_create_manual_game_v2', 'live_update_planned_game_v1', 'live_cancel_planned_game_v1',
...
print(" - Game-Day Hub supports manual Tournament Game, Scrimmage, and Friendly creation")
