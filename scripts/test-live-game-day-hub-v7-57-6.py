...
dashboard_js = (ROOT / ("js/live-dashboard-v7-57-9.js" if (ROOT / "js/live-dashboard-v7-57-9.js").exists() else ("js/live-dashboard-v7-57-8.js" if (ROOT / "js/live-dashboard-v7-57-8.js").exists() else ("js/live-dashboard-v7-57-7.js" if (ROOT / "js/live-dashboard-v7-57-7.js").exists() else "js/live-dashboard-v7-57-6.js")))).read_text()
css = (ROOT / ("css/live-sandbox-v7-57-9.css" if (ROOT / "css/live-sandbox-v7-57-9.css").exists() else ("css/live-sandbox-v7-57-8.css" if (ROOT / "css/live-sandbox-v7-57-8.css").exists() else ("css/live-sandbox-v7-57-7.css" if (ROOT / "css/live-sandbox-v7-57-7.css").exists() else "css/live-sandbox-v7-57-6.css")))).read_text()
...
check("site version", site.get("version") in {"7.57.6","7.57.7","7.57.8","7.57.9","7.57.10", "7.57.11"})
...
for key in ["liveScoringDashboardRelease","liveScoringTeamAdminRelease","liveScoringGameDayHubRelease"]:
    check(f"release marker {key}", site.get(key) in {"7.57.6","7.57.7","7.57.8","7.57.9","7.57.10", "7.57.11"})
...
check("dashboard JS wired", ('js/live-dashboard-v7-57-6.js?v=7.57.6' in dashboard_html or 'js/live-dashboard-v7-57-7.js?v=7.57.7' in dashboard_html or 'js/live-dashboard-v7-57-8.js?v=7.57.8' in dashboard_html or 'js/live-dashboard-v7-57-9.js?v=7.57.9' in dashboard_html))
check("7.57.6 CSS wired", ('css/live-sandbox-v7-57-6.css?v=7.57.6' in dashboard_html or 'css/live-sandbox-v7-57-7.css?v=7.57.7' in dashboard_html or 'css/live-sandbox-v7-57-8.css?v=7.57.8' in dashboard_html or 'css/live-sandbox-v7-57-9.css?v=7.57.9' in dashboard_html))
...
for token in [
    ...
    ('live_create_manual_game_v3' if site.get('version') in {'7.57.7','7.57.8','7.57.9','7.57.10','7.57.11'} else 'live_create_manual_game_v2'), ('live_update_planned_game_v2' if site.get('version') in {'7.57.7','7.57.8','7.57.9','7.57.10','7.57.11'} else 'live_update_planned_game_v1'), 'live_cancel_planned_game_v1',
...
print(" - Game-Day Hub foundation supports manual game creation; 7.57.7+ presents Tournament or Friendly only" if site.get("version") in {"7.57.7","7.57.8","7.57.9","7.57.10", "7.57.11"} else " - Game-Day Hub supports manual Tournament Game, Scrimmage, and Friendly creation")
