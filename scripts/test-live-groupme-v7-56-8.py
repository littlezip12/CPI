...
for token in ('release: "7.56.8"','groupMeDelivery: "connected"','mode: "connected"','supabasePublishableKey'):
...
for token in ('js/live-backend-v7-56-8.js?v=7.56.8','js/live-sandbox-v7-56-8.js?v=7.56.8','css/live-sandbox-v7-56-8.css?v=7.56.8','id="messageList"','id="pauseMessagesButton"'):
    if token not in html: fail(f"live sandbox missing {token}")

dashboard=read("live-dashboard.html")
for token in ('id="groupMeAdminPanel"','id="groupMeDisplayName"','id="groupMeSecretName"','id="groupMeEnabled"','id="saveGroupMeButton"','id="testGroupMeButton"','id="groupMeAudit"','id="dashboardDeliveryMetric"','js/live-dashboard-v7-56-8.js?v=7.56.8'):
...
dash_js=read("js/live-dashboard-v7-56-8.js")
...
guide=read("LIVE_GROUPME_SETUP_7.56.8.md")
for token in ('GROUPME_ACCESS_TOKEN_WPI_LIVE','GROUPME_BOT_ID_LAMORINDA_A_14U_BOYS','npx supabase functions deploy groupme-post','Automatic retry runs while an authorized WPI Live page remains open','manual retry'):
...
for rel in ('js/live-backend-v7-56-8.js','js/live-dashboard-v7-56-8.js','js/live-login-v7-56-8.js','js/live-password-reset-v7-56-8.js','js/live-sandbox-v7-56-8.js'):
...
if errors:
    print("WPI GROUPME DELIVERY 7.56.8 TEST FAILED")
    for error in errors: print(f" - {error}")
    sys.exit(1)
print("WPI GROUPME DELIVERY 7.56.8 TEST PASSED")
