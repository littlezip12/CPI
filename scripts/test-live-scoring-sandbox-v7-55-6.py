...
if site.get("version") != "7.55.6": fail("site version must be 7.55.6")
for key in ("liveScoringSandboxRelease", "liveScoringAuthGatewayRelease", "liveScoringMobileWorkflowRelease"):
    if site.get(key) != "7.55.6": fail(f"{key} must be 7.55.6")
...
for token in (
    ...
    'id="continueDemoButton"', 'config/live-sandbox.js?v=7.55.6',
    'js/live-login-v7-55-6.js?v=7.55.6',
    'css/live-sandbox-v7-55-6.css?v=7.55.6',
):
...
for token in (
    ...
    'id="endQuarterButton"', 'id="lineupDialog"', 'id="showSetupButton"',
    'id="lastUpdateCard"', 'id="addNoteButton"', 'id="gameControlsDetails"',
    'id="messageList"', 'id="timelineList"', 'id="summaryPanel"',
    'config/live-sandbox.js?v=7.55.6', 'js/live-sandbox-v7-55-6.js?v=7.55.6',
    'css/live-sandbox-v7-55-6.css?v=7.55.6',
):
...
for token in ('release: "7.55.6"', 'mode: "demo"', 'environment: "sandbox"', 'groupMeDelivery: "mock"'):
...
if metadata.get("environment") != "sandbox" or metadata.get("release") != "7.55.6": fail("event registry release/environment mismatch")
...
login_js = read("js/live-login-v7-55-6.js")
for token in ('signInWithPassword', 'signUp', 'wpi-live-auth-v7-55-6', 'live-sandbox.html'):
    if token not in login_js: fail(f"login runtime missing token: {token}")

js = read("js/live-sandbox-v7-55-6.js")
for token in (
...
for token in (
    ...
    'function endQuarter()', 'Direct-submit workflow',
):
...
css = read("css/live-sandbox-v7-55-6.css")
...
for rel in ("js/live-login-v7-55-6.js", "js/live-sandbox-v7-55-6.js"):
...
if errors:
    print("WPI LIVE SCORING SANDBOX 7.55.6 TEST FAILED")
    for error in errors:
        print(f" - {error}")
    sys.exit(1)

print("WPI LIVE SCORING SANDBOX 7.55.6 TEST PASSED")
...
print(" - End quarter advances directly into the next-quarter lineup while event submissions record immediately")
