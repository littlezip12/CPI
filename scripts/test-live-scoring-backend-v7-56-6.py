...
if site.get("version")!="7.56.6": fail("site version must be 7.56.6")
for key in (
    ...
    if site.get(key)!="7.56.6": fail(f"{key} must remain 7.56.6 because browser and role surfaces did not change")
for key in ("liveScoringBackendBlueprintRelease","liveScoringConnectedBackendRelease","liveScoringPersistenceRelease"):
    if site.get(key)!="7.56.6": fail(f"{key} must be 7.56.6")
...
for token in (
    ...
    'config/live-sandbox.js?v=7.56.6','js/live-login-v7-56-6.js?v=7.56.6',
    'css/live-sandbox-v7-56-6.css?v=7.56.6'
):
...
for token in (
    ...
    'config/live-sandbox.js?v=7.56.6','js/live-sandbox-v7-56-6.js?v=7.56.6',
    'css/live-sandbox-v7-56-6.css?v=7.56.6'
):
...
for token in ('release: "7.56.6"','mode: "connected"','environment: "sandbox"','groupMeDelivery: "connected"','supabasePublishableKey','allowLocalDemo: true'):
...
if metadata.get("release")!="7.56.6" or metadata.get("environment")!="sandbox": fail("event registry release/environment mismatch")
...
login_js=read("js/live-login-v7-56-6.js")
for token in ('backend.signIn','backend.signUp','requestPasswordReset','wpi-live-auth-v7-56-6','live-dashboard.html','dashboardRedirect.searchParams.set("invite", invite)','backend.registrationStatus()','New accounts require a private team invitation'):
    if token not in login_js: fail(f"login runtime missing token: {token}")

js=read("js/live-sandbox-v7-56-6.js")
for token in (
    'const RELEASE = "7.56.6"','wpi-live-sandbox-v7-56-6','wpi-live-sandbox-v7-55-9',
...
css=read("css/live-sandbox-v7-56-6.css")
for token in (
...
for rel in ("js/live-backend-v7-56-6.js","js/live-login-v7-56-6.js","js/live-dashboard-v7-56-6.js","js/live-password-reset-v7-56-6.js","js/live-sandbox-v7-56-6.js"):
...
for token in ('id="liveDashboardApp"','id="dashboardTeamName"','id="invitePanel"','id="gameHistoryList"','live-sandbox.html?new=1','js/live-dashboard-v7-56-6.js?v=7.56.6'):
    if token not in dashboard: fail(f"live-dashboard.html missing token: {token}")
reset_page=read("live-password-reset.html")
for token in ('id="passwordResetForm"','id="newPassword"','js/live-password-reset-v7-56-6.js?v=7.56.6'):
    if token not in reset_page: fail(f"live-password-reset.html missing token: {token}")
reset_js=read("js/live-password-reset-v7-56-6.js")
...
dashboard_js=read("js/live-dashboard-v7-56-6.js")
...
backend_js=read("js/live-backend-v7-56-6.js")
...
if errors:
    print("WPI CONNECTED LIVE BACKEND 7.56.6 TEST FAILED")
    for e in errors: print(f" - {e}")
    sys.exit(1)
print("WPI CONNECTED LIVE BACKEND 7.56.6 TEST PASSED")
