...
for token in ('release: "7.56.5"','groupMeDelivery: "connected"','mode: "connected"','supabasePublishableKey'):
...
for token in ('js/live-backend-v7-56-5.js?v=7.56.5','js/live-sandbox-v7-56-5.js?v=7.56.5','css/live-sandbox-v7-56-2.css?v=7.56.2','id="messageList"','id="pauseMessagesButton"'):
    if token not in html: fail(f"live sandbox missing {token}")

dashboard=read("live-dashboard.html")
for token in ('id="groupMeAdminPanel"','id="groupMeDisplayName"','id="groupMeSecretName"','id="groupMeEnabled"','id="saveGroupMeButton"','id="testGroupMeButton"','id="groupMeAudit"','id="dashboardDeliveryMetric"','js/live-dashboard-v7-56-5.js?v=7.56.5'):
...
dash_js=read("js/live-dashboard-v7-56-5.js")
...
if errors:
    print("WPI GROUPME DELIVERY 7.56.5 TEST FAILED")
    for error in errors: print(f" - {error}")
    sys.exit(1)
print("WPI GROUPME DELIVERY 7.56.5 TEST PASSED")
