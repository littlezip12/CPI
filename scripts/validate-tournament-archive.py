...
if len(archive_events)!=4 or len(archive_divisions)!=40: fail(f'Expected 4 completed events and 40 archive divisions, found {len(archive_events)} and {len(archive_divisions)}')
...
if counts.get('events')!=4 or counts.get('divisions')!=40: fail('Archive output must represent all four events and 40 divisions')
if counts.get('bankedDivisions',0)+counts.get('pendingDivisions',0)!=40: fail('Banked and pending archive division counts must reconcile')
...
for rel in ['data/tournaments/quiksilver-cup-2026.json','data/tournaments/archive/2026-boys-futures-super-finals.json','data/tournaments/archive/2026-girls-us-club-championships.json','data/tournaments/archive/2025-evan-cousineau-memorial-cup.json']:
...
print(' - 4 completed tournaments and 40 divisions remain registered for controlled archival sync')
