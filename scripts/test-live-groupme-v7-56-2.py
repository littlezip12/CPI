...
for token in ('js/live-backend-v7-56-2.js?v=7.56.2','js/live-sandbox-v7-56-2.js?v=7.56.2','css/live-sandbox-v7-56-2.css?v=7.56.2','id="messageList"','id="pauseMessagesButton"'):
    if token not in html: fail(f"live sandbox missing {token}")

dashboard=read("live-dashboard.html")
for token in ('id="groupMeAdminPanel"','id="groupMeDisplayName"','id="groupMeSecretName"','id="groupMeEnabled"','id="saveGroupMeButton"','id="testGroupMeButton"','id="groupMeAudit"','id="dashboardDeliveryMetric"','js/live-dashboard-v7-56-2.js?v=7.56.2'):
...
sandbox=read("js/live-sandbox-v7-56-2.js")
for token in ('deliverPendingMessages','retryMessage','data-retry-event','applyDeliveryStatuses','nextRetryAt','GroupMe setup needed','Connected delivery','status = "sending"'):
...
for rel in ('js/live-backend-v7-56-2.js','js/live-dashboard-v7-56-2.js','js/live-login-v7-56-2.js','js/live-password-reset-v7-56-2.js','js/live-sandbox-v7-56-2.js'):
...
if errors:
    print("WPI GROUPME DELIVERY 7.56.2 TEST FAILED")
    for error in errors: print(f" - {error}")
    sys.exit(1)
print("WPI GROUPME DELIVERY 7.56.2 TEST PASSED")
