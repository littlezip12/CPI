...
if(!js.includes('const RELEASE = "7.54.11"'))errors.push('platform runtime cache release is not 7.54.11');
const live=registry.events.filter(event=>event.migrationStatus==='platform_live').map(event=>event.id).sort();
if(JSON.stringify(live)!==JSON.stringify(['2025-evan-cousineau-memorial-cup','2026-boys-futures-super-finals','2026-girls-futures-super-finals','2026-kap7-international','2026-quiksilver-cup','2026-san-diego-county-cup'].sort()))errors.push('event selector does not expose all six migrated events');
...
console.log(' - Shared game, team, placement, and journey renderers serve six migrated events');
