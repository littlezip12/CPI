...
for(const token of ['data/tournaments/platform/runtime.js?v=7.54.8','js/tournament-platform-v7-54-0.js?v=7.54.8','data-wpi-sponsor-placement="tournament_inline"'])if(!html.includes(token))errors.push(`platform page missing ${token}`);
if(!js.includes('const RELEASE = "7.54.8"'))errors.push('platform runtime cache release is not 7.54.8');
const live=registry.events.filter(event=>event.migrationStatus==='platform_live').map(event=>event.id).sort();
if(JSON.stringify(live)!==JSON.stringify(['2025-evan-cousineau-memorial-cup','2026-boys-futures-super-finals','2026-quiksilver-cup','2026-san-diego-county-cup'].sort()))errors.push('event selector does not expose all four migrated events');
...
console.log(' - Shared game, team, placement, and journey renderers serve four migrated events');
