...
deep_links=contract.get('deepLinks',{})
req(deep_links.get('productionUniversalLinks')=='deferred-until-production-domain-is-final' or deep_links.get('status')=='deferred-until-production-domain-is-final','production deep-link domain should not be fabricated')
auth_contract=contract.get('authentication',{})
req(auth_contract.get('nativeMagicLinkCallbackStatus')=='must-be-validated-before-beta' or auth_contract.get('validationStatus') in {'awaiting-end-to-end-email-link-and-relaunch-test','validated'},'native auth callback validation marker missing')
