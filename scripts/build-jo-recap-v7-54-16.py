...
def session3_champions(bundle):
    ...
    for division in bundle.get("divisions",[]):
        first=next((x for x in bundle.get("placements",{}).get(division["id"],[]) if x.get("place")==1),None)
        if not first: continue
        team=teams.get(first.get("participantId"),first)
        rows.append({"ageGroup":division["ageGroup"],"category":division["gender"],"division":"Championship","team":first.get("name") or team.get("name")})
    return rows
def main():
    ...
    recap={
      ...
      "status":"complete_with_source_gap","generatedAt":"2026-08-02T22:28:00-07:00",
      "summary":{"weekends":3,"divisions":31,"verifiedPlacements":1093,"completeDivisions":30,"sourceIncompleteDivisions":1,"weekend3FinalGames":464,"weekend3ScheduledWithoutScores":81},
      "sourceNotice":"Weekend 3 12U Coed is the only incomplete division: the official bracket lists all 81 games, but every score cell is blank. WPI does not infer results or placements.",
      "weekends":[
       ...
       {"id":"weekend-3","label":"Weekend 3","subtitle":"Boys, Girls & Coed · North Texas","dates":"July 30–August 2, 2026","location":"North Texas","divisionCount":8,"verifiedPlacements":117,"status":"complete_with_source_gap","publicPath":"tournament.html?event=2026-jo-session-3","champions":session3_champions(s3),"sourceGap":"12U Coed: 81 scheduled games, 0 published scores"}
      ]
    }
    dump(RECAP_PATH,recap)
    print("2026 JO RECAP BUILD COMPLETE")
    print(" - 3 weekends, 31 divisions, 1,093 verified placements")
    print(" - Weekend 3: 117 placements and 464 finals across seven complete divisions")
    print(" - Weekend 3 12U Coed remains schedule-only with 81 unscored games")
    return 0
