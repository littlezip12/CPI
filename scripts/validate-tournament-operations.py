...
if config.get("release") not in {EXPECTED_RELEASE, "7.54.0", "7.54.1", "7.54.2", "7.54.3", "7.54.4", "7.54.5", "7.54.6", "7.54.7"}:
...
if len(registry.get("events", [])) != 7 or len(rows) != 71:
    fail(f"Expected 7 events and 71 divisions, found {len(registry.get('events', []))} and {len(rows)}")
...
if len(archive) != 35:
    fail(f"Expected 35 completed-event archive divisions, found {len(archive)}")
if len(historical) != 5:
...
if ops.get("counts", {}).get("archiveDivisions") != 35:
    fail("Operations output must expose all 35 verified archive divisions")
...
print(" - 7 registered tournaments and 71 divisions share one operations framework")
...
print(" - 35 completed-event divisions remain isolated in controlled archive mode; 5 Girls Club divisions remain in data review")
