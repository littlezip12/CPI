...
if site.get("version") != "7.55.4": fail("site version must be 7.55.4")
for key in ("liveScoringSandboxRelease", "liveScoringAuthGatewayRelease", "liveScoringMobileWorkflowRelease"):
    if site.get(key) != "7.55.4": fail(f"{key} must be 7.55.4")
...
for token in (
    ...
    'id="continueDemoButton"', 'config/live-sandbox.js?v=7.55.4',
    'js/live-login-v7-55-4.js?v=7.55.4',
    'css/live-sandbox-v7-55-4.css?v=7.55.4',
):
...
for token in (
    ...
    'id="messagePreview"', 'id="eventForm"', 'id="recordEventButton"',
    'id="startQuarterButton"', 'id="lineupDialog"', 'id="showSetupButton"',
    'id="messageList"', 'id="timelineList"', 'id="summaryPanel"',
    'config/live-sandbox.js?v=7.55.4', 'js/live-sandbox-v7-55-4.js?v=7.55.4',
    'css/live-sandbox-v7-55-4.css?v=7.55.4',
):
    if token not in html: fail(f"live-sandbox.html missing token: {token}")
for forbidden in ('id="clockSeconds"', 'id="clockMinutes"', 'id="actionTabs"', 'id="secondaryPlayer"', 'Second player'):
...
for token in ('release: "7.55.4"', 'mode: "demo"', 'environment: "sandbox"', 'groupMeDelivery: "mock"'):
...
if metadata.get("environment") != "sandbox" or metadata.get("release") != "7.55.4": fail("event registry release/environment mismatch")
if metadata.get("entryModel") != "dropdown": fail("event registry must use dropdown entry model")
...
login_js = read("js/live-login-v7-55-4.js")
for token in ('signInWithPassword', 'signUp', 'wpi-live-auth-v7-55-4', 'live-sandbox.html'):
    if token not in login_js: fail(f"login runtime missing token: {token}")

js = read("js/live-sandbox-v7-55-4.js")
for token in (
    'environment: "sandbox"', 'localStorage', 'live-login.html',
    'parseClock', 'clockTime', 'eventType', 'primaryPlayer', 'assistPlayer',
    'Unassisted', 'allowsAssist', 'messagesPaused', 'score_correction',
...
for forbidden in ("api.groupme.com", "GROUPME_BOT_ID", "actionTabs", '$("clockSeconds")', '$("secondaryPlayer")'):
    if forbidden in js: fail(f"browser runtime contains removed or secret token: {forbidden}")

css = read("css/live-sandbox-v7-55-4.css")
for token in (".live-scoreboard", ".live-quarter-row", ".live-event-form", ".live-preview-card", ".live-login-shell", "@media (max-width: 720px)"):
    if token not in css: fail(f"sandbox CSS missing {token}")

for rel in ("js/live-login-v7-55-4.js", "js/live-sandbox-v7-55-4.js"):
...
if errors:
    print("WPI LIVE SCORING SANDBOX 7.55.4 TEST FAILED")
    for error in errors:
        print(f" - {error}")
    sys.exit(1)

print("WPI LIVE SCORING SANDBOX 7.55.4 TEST PASSED")
print(" - Dedicated hidden sign-in/create-account gateway protects entry to the scorer")
print(" - Compact mobile flow uses one MM:SS clock, event/player dropdowns, goal-only assist, live message preview, and one submit button")
print(" - Reusable roster and quarter lineups remain available while setup is hidden during play")
