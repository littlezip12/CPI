#!/usr/bin/env python3
"""Regression checks preserving WPI Live 7.58.7 Club Pilot Validation under 7.58.8."""
...
req(read('VERSION.md').strip()=='# WPI 7.58.8 — Club-Branded Game Experience','current VERSION mismatch')
req(site.get('version')=='7.58.8','current release must be 7.58.8')
...
req(index.get('release')=='7.58.8','schedule index release mismatch')
req(index.get('activeCompetitiveSeason')=='2026-2027','active competitive season changed')
req(index.get('counts')=={'events':0,'games':0},'7.58.8 must not fabricate an official current-season schedule')
...
print('WPI LIVE 7.58.7 CLUB PILOT VALIDATION REGRESSION PASSED UNDER 7.58.8')
