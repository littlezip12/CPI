...
if site.get("version")!="7.56.1": fail("site version must be 7.56.1")
for key in (
    ...
    if site.get(key)!="7.56.1": fail(f"{key} must be 7.56.1")
for key in ("liveScoringBackendBlueprintRelease","liveScoringConnectedBackendRelease","liveScoringDashboardRelease","liveScoringRoleSecurityRelease","liveScoringPersistenceRelease"):
    if site.get(key)!="7.56.1": fail(f"{key} must be 7.56.1")
...
for token in (
    ...
    'config/live-sandbox.js?v=7.56.1','js/live-login-v7-56-1.js?v=7.56.1',
    'css/live-sandbox-v7-56-1.css?v=7.56.1'
):
...
for token in (
    ...
    'config/live-sandbox.js?v=7.56.1','js/live-sandbox-v7-56-1.js?v=7.56.1',
    'css/live-sandbox-v7-56-1.css?v=7.56.1'
):
...
for token in ('release: "7.56.1"','mode: "demo"','environment: "sandbox"','groupMeDelivery: "mock"','supabasePublishableKey','allowLocalDemo: true'):
...
if metadata.get("release")!="7.56.1" or metadata.get("environment")!="sandbox": fail("event registry release/environment mismatch")
...
login_js=read("js/live-login-v7-56-1.js")
for token in ('backend.signIn','backend.signUp','requestPasswordReset','wpi-live-auth-v7-56-1','live-dashboard.html','dashboardRedirect.searchParams.set("invite", invite)','backend.registrationStatus()','New accounts require a private team invitation'):
    if token not in login_js: fail(f"login runtime missing token: {token}")

js=read("js/live-sandbox-v7-56-1.js")
for token in (
    'const RELEASE = "7.56.1"','wpi-live-sandbox-v7-56-1','wpi-live-sandbox-v7-55-9',
    ...
    'workspace.role === "viewer" && !remoteGameId','workspace.defaultLineupPlayerIds','remoteToLocal','createdByUserId'
):
...
css=read("css/live-sandbox-v7-56-1.css")
for token in (
...
for rel in ("js/live-backend-v7-56-1.js","js/live-login-v7-56-1.js","js/live-dashboard-v7-56-1.js","js/live-password-reset-v7-56-1.js","js/live-sandbox-v7-56-1.js"):
...
for token in ('id="liveDashboardApp"','id="dashboardTeamName"','id="invitePanel"','id="gameHistoryList"','live-sandbox.html?new=1','js/live-dashboard-v7-56-1.js?v=7.56.1'):
    if token not in dashboard: fail(f"live-dashboard.html missing token: {token}")
reset_page=read("live-password-reset.html")
for token in ('id="passwordResetForm"','id="newPassword"','js/live-password-reset-v7-56-1.js?v=7.56.1'):
    if token not in reset_page: fail(f"live-password-reset.html missing token: {token}")
reset_js=read("js/live-password-reset-v7-56-1.js")
...
dashboard_js=read("js/live-dashboard-v7-56-1.js")
...
backend_js=read("js/live-backend-v7-56-1.js")
for token in ('class WPILiveBackend','live_registration_status','registrationStatus()','live_bootstrap_workspace','live_players','live_games','live_lineups','live_events','live_game_recaps','state_snapshot','subscribeToGame','publishable keys only','Viewer accounts are read-only','createdByUserId','updated_by'):
...
full_sql=read("supabase/WPI_LIVE_7_56_1_FULL_SETUP.sql")
...
setup=read("LIVE_BACKEND_SETUP_7.56.1.md")
for token in ('Project URL','Publishable key','WPI_LIVE_7_56_1_FULL_SETUP.sql','Owner','Admin','Scorer','Viewer','Never copy a secret key'):
...
if errors:
    print("WPI CONNECTED LIVE BACKEND 7.56.1 TEST FAILED")
    for e in errors: print(f" - {e}")
    sys.exit(1)
print("WPI CONNECTED LIVE BACKEND 7.56.1 TEST PASSED")
...
print(" - Authorized realtime resume and browser-local fallback are included; GroupMe remains mock")
