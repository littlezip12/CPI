...
if site.get("version")!="7.56.5": errors.append("site version must be 7.56.5")
if site.get("liveScoringMultiDeviceRelease")!="7.56.5": errors.append("multi-device release marker missing")
if site.get("liveScoringRoleSecurityRelease")!="7.56.5": errors.append("role-security release marker missing")
...
if '!["owner", "admin", "scorer"].includes(membership.role)' not in edge:
    errors.append("Edge Function no longer authorizes Scorer play delivery")
...
for page,script in (
    ...
    text=read(page)
    if 'live-backend-v7-56-5.js?v=7.56.5' not in text: errors.append(f"{page} missing 7.56.5 backend")
    if script not in text: errors.append(f"{page} missing {script}")
    if 'config/live-sandbox.js?v=7.56.5' not in text: errors.append(f"{page} missing 7.56.5 config cache bust")

for rel in ("js/live-backend-v7-56-5.js","js/live-dashboard-v7-56-5.js","js/live-login-v7-56-5.js","js/live-password-reset-v7-56-5.js","js/live-sandbox-v7-56-5.js"):
...
if errors:
    print("WPI SCORER MULTI-DEVICE 7.56.5 TEST FAILED")
    for error in errors: print(f" - {error}")
    sys.exit(1)
print("WPI SCORER MULTI-DEVICE 7.56.5 TEST PASSED")
...
print(" - Private WPI Live assets are cache-busted to 7.56.5")
