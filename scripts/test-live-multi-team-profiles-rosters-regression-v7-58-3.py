#!/usr/bin/env python3
"""Protect the validated 7.58.2 profile/roster foundation during 7.58.3."""
...
req(release.get('version')=='7.58.3','current release must be 7.58.3')
...
for token in ('backend.updateTeamProfile','backend.saveRosterVersion','backend.setDefaultLineup','renderRosterVersions','renderDefaultLineup'):
    req(token in dashboard,f'7.58.2 behavior missing from 7.58.3 dashboard: {token}')
