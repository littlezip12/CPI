...
req(site.get('version') in {'7.64.4','7.64.5'},'site release must preserve 7.64.4 or later')
req(any(v in version for v in ('WPI 7.64.4','WPI 7.64.5')),'VERSION missing 7.64.4+')
...
req('input.dispatchEvent(new Event("blur"))' in clock,'clock confirm must sync through existing protected blur handler')
req('645' in clock and 'minutes>15' in clock,'clock parser must preserve existing shorthand/range rules')
