...
if site.get("tournamentPlatformRelease") not in {"7.54.1", "7.54.2", "7.54.3", "7.54.4", "7.54.5", "7.54.6", "7.54.7"}:
    errors.append("tournamentPlatformRelease must preserve 7.54.1")
if site.get("tournamentRegistryRelease") not in {"7.54.1", "7.54.2", "7.54.3", "7.54.4", "7.54.5", "7.54.6", "7.54.7"}:
...
if site.get("version") not in {"7.54.1", "7.54.2", "7.54.3", "7.54.4", "7.54.5", "7.54.6", "7.54.7"}:
    errors.append("site version must preserve the Boys Futures migration")
if registry.get("release") not in {"7.54.1", "7.54.2", "7.54.3", "7.54.4", "7.54.5", "7.54.6", "7.54.7"} or len(registry.get("events", [])) != 7:
    errors.append("platform registry must contain seven registered events")
platform_live = [event for event in registry.get("events", []) if event.get("migrationStatus") == "platform_live"]
if {event.get("id") for event in platform_live} != {"2026-quiksilver-cup", "2026-boys-futures-super-finals", "2025-evan-cousineau-memorial-cup"}:
    errors.append("Quiksilver, Boys Futures, and Evan Cousineau must be the three platform-live events")
...
for token in ["data/tournaments/platform/runtime.js?v=7.54.7", "js/tournament-platform-v7-54-0.js?v=7.54.7"]:
