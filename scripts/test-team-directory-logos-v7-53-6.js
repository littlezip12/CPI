...
const semverAtLeast = (value, floor) => { const a=String(value||'').split('.').map(Number), b=String(floor).split('.').map(Number); for(let i=0;i<3;i++){ const av=a[i]||0,bv=b[i]||0; if(av>bv)return true; if(av<bv)return false; } return true; };
if (!semverAtLeast(site.version,'7.53.7')) fail('site version must preserve the 7.53.6 logo release');
