...
if site.get("version") != "7.55.8": fail("site version must be 7.55.8")
for key in ("liveScoringSandboxRelease", "liveScoringAuthGatewayRelease", "liveScoringMobileWorkflowRelease"):
    if site.get(key) != "7.55.8": fail(f"{key} must be 7.55.8")
...
for token in (
    ...
    'id="continueDemoButton"', 'config/live-sandbox.js?v=7.55.8',
    'js/live-login-v7-55-8.js?v=7.55.8',
    'css/live-sandbox-v7-55-8.css?v=7.55.8',
):
...
for token in (
    'name="robots" content="noindex,nofollow,noarchive"',
    'id="setupPanel"', 'id="rosterList"', 'id="rosterHeading"', 'id="rosterStatus"', 'id="liveConsole"',
    'id="clockTime"', 'placeholder="MM:SS"', 'id="eventType"',
    'id="primaryPlayer"', 'id="assistPlayer"', 'Unassisted',
    'id="messagePreview"', 'id="messagePreviewDetails"', 'id="eventForm"', 'id="recordEventButton"', 'Submit play',
    'id="endQuarterButton"', 'id="lineupDialog"', 'id="lineupCloseButton"', 'id="lineupCancelButton"', 'id="showSetupButton"',
    'id="lastUpdateCard"', 'id="addNoteButton"', 'id="gameControlsDetails"',
    'id="messageList"', 'id="timelineList"', 'id="summaryPanel"',
    'id="resetSandboxTopButton"', 'id="resetSandboxGameButton"', 'id="resetSandboxButton"', 'Reset test game',
    'config/live-sandbox.js?v=7.55.8', 'js/live-sandbox-v7-55-8.js?v=7.55.8',
    'css/live-sandbox-v7-55-8.css?v=7.55.8',
):
...
for token in ('release: "7.55.8"', 'mode: "demo"', 'environment: "sandbox"', 'groupMeDelivery: "mock"'):
...
if metadata.get("environment") != "sandbox" or metadata.get("release") != "7.55.8": fail("event registry release/environment mismatch")
...
login_js = read("js/live-login-v7-55-8.js")
for token in ('signInWithPassword', 'signUp', 'wpi-live-auth-v7-55-8', 'live-sandbox.html'):
    if token not in login_js: fail(f"login runtime missing token: {token}")

js = read("js/live-sandbox-v7-55-8.js")
for token in (
    'environment: "sandbox"', 'localStorage', 'live-login.html',
    'parseClock', 'clockTime', 'eventType', 'primaryPlayer', 'assistPlayer',
    'Unassisted', 'allowsAssist', 'endQuarterButton', 'lastUpdateCard',
    'addNoteButton', 'messagesPaused', 'score_correction',
    'downloadLog', 'buildRecap', 'showSetup', 'hideSetup',
    '@supabase/supabase-js@2.110.8/+esm', 'function resetSandbox()',
    'Test game reset. Team roster and saved starters were preserved.',
):
    if token not in js: fail(f"sandbox runtime missing token: {token}")
for forbidden in ("api.groupme.com", "GROUPME_BOT_ID", "actionTabs", '$("clockSeconds")', '$("secondaryPlayer")', '$("startQuarterButton")', '$("startNextQuarterButton")'):
    if forbidden in js: fail(f"browser runtime contains removed or secret token: {forbidden}")

for token in (
    'starterRule', 'ageGroup', 'defaultLineup', 'defaultGoalieId', 'lineupGoalies',
    'lineupGoalie', 'lineupCount', 'Select exactly one goalie', 'between_quarters',
    'function endQuarter()', 'Submit-play workflow', 'function addRosterPlayer()', 'Lamorinda A 14U Boys', 'Submit play',
):
    if token not in js: fail(f"age-aware lineup/quarter flow missing token: {token}")
if 'confirm(' in js[js.find('function recordSelectedEvent'):js.find('function undoLastEvent')]:
    fail("event submission must not use a second confirmation")
if metadata.get("lineupRules", {}).get("12U", {}).get("totalStarters") != 6:
    fail("12U starter rule must require six total")
if metadata.get("lineupRules", {}).get("14U", {}).get("totalStarters") != 7:
    fail("14U starter rule must require seven total")
if metadata.get("mobileWorkflow", {}).get("confirmationStep") is not False:
    fail("event confirmation step must be disabled")
workflow = metadata.get("mobileWorkflow", {})
if workflow.get("submitLabel") != "Submit play": fail("submit label must be Submit play")
if workflow.get("postSubmitPreserve") != ["time_remaining", "quarter"]: fail("quarter and time must persist after submission")
if workflow.get("endQuarterClock") != "0:00": fail("end quarter must record at 0:00")
if metadata.get("pilotTeam") != "Lamorinda A 14U Boys": fail("pilot team mismatch")
reset = workflow.get("resetTestGame", {})
if reset.get("returnsTo") != "pregame_setup": fail("reset must return to pregame setup")
if "roster" not in reset.get("preserves", []) or "default_starting_lineup" not in reset.get("preserves", []): fail("reset must preserve roster and default lineup")
if "events" not in reset.get("clears", []) or "mock_groupme_log" not in reset.get("clears", []): fail("reset must clear events and mock GroupMe log")

css = read("css/live-sandbox-v7-55-8.css")
...
for rel in ("js/live-login-v7-55-8.js", "js/live-sandbox-v7-55-8.js"):
...
if errors:
    print("WPI LIVE SCORING SANDBOX 7.55.8 TEST FAILED")
    for error in errors:
        print(f" - {error}")
    sys.exit(1)

print("WPI LIVE SCORING SANDBOX 7.55.8 TEST PASSED")
print(" - Dedicated hidden sign-in/create-account gateway protects entry to the scorer")
print(" - Exact age-aware starter rules and reusable starting lineups are enforced")
print(" - Reset test game is reachable before, during, and after play while preserving roster and saved starters")
print(" - Add player works, the roster follows Lamorinda A 14U Boys, and Submit play resets only play-specific fields")
print(" - End quarter records the score at 0:00 and requires the next-quarter lineup before scoring resumes")
