...
for page,script in (
    ...
    text=read(page)
    if 'live-backend-v7-56-6.js?v=7.56.6' not in text: errors.append(f"{page} missing backend cache bust")
    if script not in text: errors.append(f"{page} missing {script}")

if 'release: "7.56.6"' not in config: errors.append('active live config missing release 7.56.6')
if release.get('version') != '7.56.6': errors.append('site-release version is not 7.56.6')

for path in ("js/live-backend-v7-56-6.js","js/live-sandbox-v7-56-6.js","js/live-dashboard-v7-56-6.js","js/live-login-v7-56-6.js","js/live-password-reset-v7-56-6.js"):
...
if errors:
    print("WPI LIVE EVENT DELIVERY 7.56.6 TEST FAILED")
    for error in errors: print(f" - {error}")
    sys.exit(1)

print("WPI LIVE EVENT DELIVERY 7.56.6 TEST PASSED")
...
print(" - WPI Live 7.56.6 browser assets remain protected with cross-account Scorer persistence protected")
