...
function loadJourneyContext(side){
  ...
  requireCondition(app.includes("APP_VERSION='7.50.7'"),`${side} does not advertise APP_VERSION 7.50.7`);
...
for(const [label,ctx] of [['Girls',girls],['Boys',boys]]){
  const live=labeledPoolRouteGames.map(g=>({...g,game:label==='Boys'?String(g.game):g.game}));
...
console.log('JO JOURNEY 7.50.7 TESTS PASSED');
