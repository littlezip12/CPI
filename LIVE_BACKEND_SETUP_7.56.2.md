# WPI 7.56.2 — Connected Live Backend Setup
...
WPI 7.56.2 includes real server-side GroupMe delivery, but it remains inactive until the connected backend is tested, a dedicated GroupMe test bot is created, the bot ID is stored as a Supabase Edge Function secret, and the function is deployed. Follow `LIVE_GROUPME_SETUP_7.56.2.md` only after the authentication and multi-device storage test succeeds.
...
python3 scripts/configure-live-backend-v7-56-2.py \
...
```bash
python3 scripts/configure-live-backend-v7-56-2.py --demo
```
...
## After backend activation

Follow `LIVE_GROUPME_SETUP_7.56.2.md` to activate the dedicated test-group destination. The next planned product release is WPI 7.56.3 for approved recap styles and cross-game analytics after the connected backend and GroupMe pilot are stable.
