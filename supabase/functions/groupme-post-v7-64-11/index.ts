// WPI 7.64.11 — authenticated delivery + Final Whistle team-stats image attachment + self-service setup.
  ...
  try {
    ...
    if (action === "upload_stats_card") {
      ...
      const upload = await fetch("https://image.groupme.com/pictures",{
        method:"POST",
        headers:{"X-Access-Token":credential,"Content-Type":match[1]==="jpeg"?"image/jpeg":"image/png","User-Agent":"WPI-Live/7.64.11"},
        body:binary,
        signal:AbortSignal.timeout(15000),
      });
    ...
    const { data: event, error: eventError } = await adminClient
      .from("live_events")
      .select("id,message_text,status,metrics,game_id,game:live_games!inner(id,team_id,environment,status,messages_paused,message_frequency,destination_id,destination:live_destinations(id,display_name,enabled,delivery_mode,groupme_topic_id,groupme_topic_name))")
      .eq("id", eventId)
      .single();
