...
-- Temporarily unlocks detailed Team Insights for authenticated Supporters/followers
-- while preserving paid-entitlement metadata, future Stripe plumbing and ad-free
-- treatment for actual paid/operational users. No payment is collected.
...
begin
  ...
  return public.live_is_team_follower(target_team_id)
    or exists(
      select 1 from public.live_team_members m
      where m.team_id=target_team_id and m.user_id=caller
    );
end;
$$;
