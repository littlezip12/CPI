...
-- Adds an Owner/Admin read-only diagnostic view for the remaining Club Pilot
-- proving gates. This does not grant new team permissions, change scoring,
-- mutate GroupMe delivery, or mark 7.59.0 ready automatically.
...
begin
  ...
  return jsonb_build_object(
    ...
    'gates',jsonb_build_array(
      ...
      jsonb_build_object(
        'key','offline_reconnect','state','manual',
        'title','Offline → reconnect','detail','Manual device test still required; browser state is game-scoped but the network transition must be exercised in real use'
      ),
      jsonb_build_object(
...
comment on function public.live_club_pilot_validation_v1(uuid) is
  'Owner/Admin read-only observability for Club Pilot proving gates. Evidence is derived from canonical WPI Live records and does not itself declare 7.59.0 ready.';
